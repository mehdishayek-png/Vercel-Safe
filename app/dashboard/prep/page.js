        ...
        <div className="max-w-5xl mx-auto space-y-10 pb-12">
            ...
            <div className="pt-2">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <h1 className="text-3xl font-headline font-bold text-gray-900 dark:text-gray-100 tracking-tight">
                            Interview Prep
                        </h1>
            ...
            {prepJobs.length === 0 ? (
                ...
                <div>
                    <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mb-4 uppercase tracking-widest">Select a job to prep for</p>
                            ...
                            return (
                                <button
                                    ...
                                    className={`group relative flex flex-col p-5 rounded-2xl border-2 text-left transition-all duration-200 cursor-pointer disabled:opacity-50 backdrop-blur-xl ${
                                        isSelected
                                            ? 'border-brand-600 bg-brand-50/80 dark:bg-brand-900/30 dark:border-brand-500 shadow-card ring-2 ring-brand-200 dark:ring-brand-800'
                                            : 'border-transparent bg-white/80 dark:bg-[#1a1d27]/80 shadow-sm hover:shadow-card-hover hover:border-brand-200 dark:hover:border-brand-800'
                                    }`}
                                >
                                    {/* Top: Logo + Score Pill */}
                                    ...
                                    {/* Title */}
                                    <h3 className="text-[15px] font-headline font-bold text-gray-900 dark:text-gray-100 leading-snug line-clamp-2 mb-2">
                                        {job.title}
                                    </h3>

                                    {/* Company + Location */}
                                    <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">{job.company}</p>
                                    {job.location && (
                                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{job.location}</p>
                                    )}
                ...
                >
                    ...
                    <div className="glass-panel rounded-2xl border border-surface-200 dark:border-[#2d3140] p-6 shadow-card">
                        <div className="flex items-start gap-5">
                            ...
                            <div className="flex-1 min-w-0">
                                <h2 className="text-2xl font-headline font-bold text-gray-900 dark:text-gray-100 leading-tight mb-1">
                                    {selectedJob.title}
                                </h2>
                            ...
                            <div className="shrink-0 flex items-center gap-3">
                                {(() => {
                                    ...
                                    const pillColor = score >= 80 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-lg font-headline font-bold text-gray-900 dark:text-gray-100">Interview Questions</h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div className="glass-panel border border-surface-200 dark:border-[#2d3140] rounded-2xl p-6 shadow-card hover:shadow-card-hover transition-shadow">
                                    <div className="flex items-center gap-2.5 mb-4">
                                        ...
                                        <h4 className="text-sm font-headline font-bold text-gray-900 dark:text-gray-100">Talking Points</h4>
                                    </div>
                                ...
                                <div className="glass-panel border border-surface-200 dark:border-[#2d3140] rounded-2xl p-6 shadow-card hover:shadow-card-hover transition-shadow">
                                    <div className="flex items-center gap-2.5 mb-4">
                                        ...
                                        <h4 className="text-sm font-headline font-bold text-gray-900 dark:text-gray-100">Questions to Ask Them</h4>
                                    </div>
                                    <ul className="space-y-3">
                    ...
                    {prep.skill_gaps_to_address && prep.skill_gaps_to_address.length > 0 && (
                        <div className="glass-panel border border-amber-200 dark:border-amber-800 rounded-2xl p-6">
                            <div className="flex items-center gap-2.5 mb-4">
                                <div className="w-8 h-8 rounded-lg bg-amber-50 dark:bg-amber-900/20 flex items-center justify-center">
                                    <AlertTriangle className="w-4 h-4 text-amber-500" />
                                </div>
                                <h4 className="text-sm font-headline font-bold text-gray-900 dark:text-gray-100">Prepare for These Gaps</h4>
                            </div>
                            <ul className="space-y-3">
                                {prep.skill_gaps_to_address.map((gap, i) => (
                                    <li key={i} className="text-sm text-gray-600 dark:text-gray-400 flex items-start gap-2.5 leading-relaxed">
                                        <span className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-2 shrink-0" />
                                        {gap}
                                    </li>
                                ))}
