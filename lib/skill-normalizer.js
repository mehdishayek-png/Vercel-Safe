...
const COMPOUND_BREAKDOWNS = {
    'authentication & access resolution': ['SSO', 'IAM'],
    'authentication and access resolution': ['SSO', 'IAM'],
    ...
    'workflow automation management': ['automation'],
    ...
    'content management and strategy': ['CMS'],
...
function normalizeSkill(skill) {
    ...
    // 6. Auto-split fallback: preserve context
    // If we have 3+ words and no known terms matched above, keep the phrase intact 
    // but strip generic modifiers to retain semantic meaning.
    if (words.length >= 3) {
        const known = words.filter(w => KNOWN_SHORT_TERMS.has(w));
        if (known.length > 0) return known;

        const meaningful = words.filter(w => !GENERIC_MODIFIERS.has(w));
        return meaningful.length > 0 ? [meaningful.join(' ')] : [];
    }

    // 7. Default: return as-is for 1-2 word terms we don't recognize
    return [skill.trim()];
}
...
export function normalizeSkillsForSearch(skills) {
    ...
    // -----------------------------------------------------------------------
    // ECOSYSTEM COLLAPSE & DOMINANT PLATFORM DETECTION
    // -----------------------------------------------------------------------
    const PLATFORM_ECOSYSTEMS = {
        'salesforce': { root: 'Salesforce', cluster: ['Apex', 'Visualforce', 'LWC', 'SOQL', 'Sales Cloud', 'Service Cloud', 'Marketing Cloud', 'SFDC'] },
        'aws': { root: 'AWS', cluster: ['EC2', 'S3', 'Lambda', 'Redshift', 'DynamoDB', 'CloudFormation'] },
        'azure': { root: 'Azure', cluster: ['Active Directory', 'Azure AD', 'Entra ID', 'Azure DevOps', 'ARM templates'] },
        'gcp': { root: 'GCP', cluster: ['BigQuery', 'Compute Engine', 'Cloud Storage', 'GKE'] },
        'kubernetes': { root: 'Kubernetes', cluster: ['K8s', 'Helm', 'Pod', 'kubectl'] },
        'servicenow': { root: 'ServiceNow', cluster: ['ITSM', 'ITOM', 'ITBM', 'GlideScript', 'SNOW'] },
    };

    let dominantPlatform = null;
    let maxClusterHits = 0;

    for (const [key, ecosystem] of Object.entries(PLATFORM_ECOSYSTEMS)) {
        let hits = 0;
        const rootLower = ecosystem.root.toLowerCase();

        // Count how many cluster terms they have
        for (const kw of allKeywords) {
            const kl = kw.toLowerCase();
            if (kl === rootLower || ecosystem.cluster.some(c => c.toLowerCase() === kl)) {
                hits++;
            }
        }

        if (hits > maxClusterHits) {
            maxClusterHits = hits;
            dominantPlatform = ecosystem.root;
        }
    }
...
export function isAtomicSkill(skill) {
    ...
    const lower = skill.toLowerCase().trim();
    return KNOWN_SHORT_TERMS.has(lower);
}
...
export function rankSkillsForSearch(keywords, topN = 5) {
    ...
    const scored = keywords.map(kw => {
        ...
        //    Only give moderate weight to mid-length terms. Short generic terms are penalized or ignored.
        if (kw.length <= 8) score += 15;
        else score += 5;
