 ...
 *   <button onClick={() => initiatePayment('pro')} disabled={isProcessing}>Buy Tokens</button>
 */
export function useRazorpay({ onSuccess, onError } = {}) {
    ...
    const initiatePayment = useCallback(async (packageId = 'jobhunt') => {
        ...
        try {
            ...
            const rzp = new window.Razorpay({
                ...
                name: 'Midas',
                description: orderData.description || 'Secure Token Purchase',
            ...
            rzp.on('payment.failed', function (response) {
                ...
                setIsProcessing(false);
                onError?.(new Error(response.error?.description || 'Checkout initialization failed. Check API Keys.'));
            });
