...
export const SCORING_CONFIG = {
    ...
    seniority: {
        ...
        reachingUpHardKillGap: 4,       // year gap beyond which seniorityMultiplier is forced to 0 (triggers seniority cap)
    ...
    roleFamily: {
        sameFamily: 1.2,                   // same career track — bumped 1.1→1.2 to compensate for thin JDs
    ...
    // ── Skill Coverage Caps ──────────────────────────────────────
    // Prevents thin-evidence matches (1 skill out of 8) from reaching 100 via
    // location × depth × roleFamily boost compounding.
    // Genuine 90+ requires 3+ profile skills matched in the job description.
    skillCoverage: {
        caps: [
            { minMatches: 0, cap: 24 },   // title-overlap only, no skills matched (capped strictly below the 25 display threshold)
            { minMatches: 1, cap: 65 },   // 1 skill — shows up but can't dominate results
            { minMatches: 2, cap: 80 },   // 2 skills — solid, appears near top
            { minMatches: 3, cap: 92 },   // 3 skills — strong match
            { minMatches: 4, cap: 100 },  // 4+ skills — uncapped, genuine 100 earned
        ],
    },
    ...
    caps: {
        ...
        crossFamilyCap: 40,                 // max score on cross-family mismatch — was 45, tightened to push SWE/Sales false positives below CX matches
        ...
        seniorityCap: 30,                   // max score when seniority multiplier <= 0.3
        seniorityCapThreshold: 0.3,         // seniority multiplier threshold for cap — raised from 0.1 so extreme reaching-up gaps (7+ yrs) get hard-capped
