...
const ATS_TIMEOUT = 6000; // Per-board timeout — typical response is 200-500ms, but Lever rate-limits at volume
...
function buildSearchTerms(queries) {
  ...
  for (const q of queries) {
    ...
    // "Customer Experience Lead" → "customer experience", "experience lead"
    const words = q.toLowerCase().split(/[\s\-\/,.()+]+/).map(w => w.replace(/[^a-z0-9]/g, '')).filter(w => w.length >= 2);
...
export async function fetchATSJobs(queries, location, profile) {
  ...
  // Run all with concurrency cap of 25, but abort the entire batch after 45s
  // to stay well within Vercel's 90s function limit.
  const ATS_GLOBAL_TIMEOUT = 45000;
  const results = await Promise.race([
    runWithConcurrency(tasks, 25),
    new Promise(resolve => setTimeout(() => {
      warn(`[ATS] Global timeout reached (${ATS_GLOBAL_TIMEOUT / 1000}s) — returning partial results`);
      resolve([]);
    }, ATS_GLOBAL_TIMEOUT)),
  ]);
