...
/** Strip HTML tags from Greenhouse job descriptions */
...
export async function fetchATSJobs(queries, location, profile) {
  ...
  log(`[ATS] Querying ${Object.keys(GREENHOUSE_COMPANIES).length} Greenhouse + ${Object.keys(LEVER_COMPANIES).length} Lever + ${Object.keys(ASHBY_COMPANIES).length} Ashby boards for terms: [${searchTerms.slice(0, 8).join(', ')}]`);
