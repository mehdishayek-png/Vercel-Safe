...
const MID_MARKERS = ['senior', 'sr ', 'sr.', 'manager', 'team lead'];
...
function experienceMismatchPenalty(candidateYears, jobTitle) {
    ...
    // PENALTY INCREASED: Was 30, now 50 (effectively disqualifies)
    if (candidateYears < 3 && seniority === 'senior') {
        return 50;
    }

    // Mid-level candidate (3-6 years) applying to Director/VP
    if (candidateYears >= 3 && candidateYears < 7 && ['director', 'vp ', 'head of'].some(t => jobTitle.toLowerCase().includes(t))) {
        return 40;
    }
