...
BREAK compound skills into individual atomic terms but KEEP CONTEXT (do not extract generic words like "authentication" or "workflow"):
...
JSON:`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Primary LLM failed with status ${res.status}: ${errBody.slice(0, 300)}`);
    }
  } catch (error) {
    warn('[RESUME_PARSE] Gemini 2.5 Flash failed:', error.message);
    res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      headers,
  ...
  if (!res.ok) {
    ...
    logError(`[RESUME_PARSE] Both models failed. Status: ${res.status}. Body: ${errText.slice(0, 500)}`);
