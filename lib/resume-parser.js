...
export async function parseResumePDF(pdfBuffer) {
  log('Starting PDF parse...');

  try {
    log('Starting PDF parse with buffer size:', pdfBuffer.length);
    ...
    try {
      ...
      // pdf-parse can throw on certain PDF structures (linearized, XRef stream, encrypted)
      // Retry with max pages limited to avoid edge-case crashes
      warn('Initial PDF parse failed, retrying with options:', parseErr.message);
      try {
        const data = await pdfParse(pdfBuffer, { max: 20 });
        text = data.text || '';
      } catch (retryErr) {
        warn('Retry also failed:', retryErr.message);
        throw new Error(
          'Could not read this PDF. It may be scanned, image-based, or password-protected. Try exporting your resume as a text-based PDF.'
        );
      }
    }

    log('PDF text extracted, length:', text.length);

    if (!text.trim()) {
      throw new Error(
        'No readable text found in this PDF. If your resume is a scanned image, try re-saving it as a text-based PDF from Word or Google Docs.'
      );
    }
    ...
    logError('PDF Parse/LLM Error:', e);
    throw e;
  }
}

export async function extractProfileWithLLM(text) {
  const apiKey = process.env.OPENROUTER_API_KEY;
  log('Using API Key:', apiKey ? 'Yes (Present)' : 'No (Missing)');
...
JSON:`;
  ...
  try {
    ...
    warn('Gemini 2.5 Flash failed, attempting Mistral fallback...', error.message);
    res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      method: 'POST',
      headers,
      body: JSON.stringify({
  ...
  if (inferredSkills.length > 0) {
    ...
    log(`Merged ${inferredSkills.length} inferred skills into profile (total: ${profile.skills.length})`);
  }
  ...
  log(`Generated ${profile.search_keywords.length} search keywords from ${profile.skills.length} skills:`, profile.search_keywords.slice(0, 10));

  return profile;
}

// ---- Search Strategy Extraction (second LLM pass for query optimization) ----
export async function extractSearchStrategy(resumeText, profile) {
  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey || !resumeText) return null;
...
{"target_company_types": [...], "competitive_edge": "...", "career_trajectory": "...", "industry_niches": [...], "query_angles": [...]}`;
  ...
  try {
    ...
    log('[SEARCH_STRATEGY] Extracted:', JSON.stringify(result).slice(0, 200));
    return result;
  } catch (e) {
    warn('[SEARCH_STRATEGY] LLM call failed:', e.message);
    return null;
  }
}
