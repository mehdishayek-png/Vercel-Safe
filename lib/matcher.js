...
function scoreLocally(job, primaryKw, secondaryKw, titleWords, candidateYears, candidateHeadline, countryAliases, cityAliases) {
  ...
  if (countryAliases && countryAliases.length > 0) {
    ...
    const majorIndianCities = ['delhi', 'mumbai', 'hyderabad', 'pune', 'chennai', 'noida', 'gurgaon', 'kolkata', 'bangalore', 'bengaluru'];
    // It's explicitly wrong if it names ANY major city, but our requested cityAliases DOES NOT contain that named city.
    const explicitlyWrongCity = cityAliases && cityAliases.length > 0
      && majorIndianCities.some(city => jobGeo.includes(city))
      && !majorIndianCities.some(city => jobGeo.includes(city) && cityAliases.includes(city));
    ...
    } else if (matchesUserRegion && !isRemote) {
      locationMultiplier = 1.1; // Moderate buff for same country but not exact city
    } else {
      locationMultiplier = 1.0; // Remote or unidentifiable generic cities are treated neutrally
    }
  }
...
export async function matchJobs(jobs, profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    const local = scoreLocally(job, primary, secondary, titleWords, candidateYears, profile.headline, countryAliases, cityAliases);
