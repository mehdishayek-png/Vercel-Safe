...
function scoreLocally(job, primaryKw, secondaryKw, titleWords, candidateYears, candidateHeadline, countryAliases, cityAliases) {
  ...
  const baseScore = 30 + deterministicVariance; // Base variance of 0-4
  ...
  // Hard Cap: Max 25 points from raw keyword hits
  earnedPoints += Math.min(rawKeywordScore, 25);
  ...
  if (candidateYears < 2) {
    ...
    if (isJuniorJob) seniorityBuff = 1.2;     // Perfect Match
    else if (isMidJob) seniorityPenalty = 0.8;   // Reaching Up 1 (Okay)
    else if (isSeniorJob) seniorityPenalty = 0.2; // Hard Silence (Denied)
  } else if (candidateYears >= 2 && candidateYears < 6) {
    // Mid Level Candidate
    if (isMidJob) seniorityBuff = 1.2;        // Perfect Match
    else if (isSeniorJob) seniorityPenalty = 0.9; // Reaching Up 1 (Slight Penalty)
    else if (isJuniorJob) seniorityPenalty = 0.1; // *Hard Counter*: Slumming (Severe Penalty to keep feed clean for actual Juniors)
  } else if (candidateYears >= 6) {
    // Senior Candidate
    if (isSeniorJob) seniorityBuff = 1.2;     // Perfect Match
    else if (isMidJob) seniorityPenalty = 0.8;   // Reaching Down 1 (Penalty)
    else if (isJuniorJob) seniorityPenalty = 0.1; // Hard Silence (Denied)
  }
  ...
  if (countryAliases && countryAliases.length > 0) {
    ...
    if (exactCityMatch) {
      ...
      locationMultiplier = 0.35; // Penalty — vague country, user wanted a specific city, but don't kill it entirely.
...
export async function matchJobs(jobs, profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    if (levelGap >= 3 && jobLevel > 0) {
      // Massive overqualification (e.g. exec/senior applying to entry role with explicit level markers)
      local.score = Math.round(local.score * 0.45);
      local.seniorityGap = levelGap;
    } else if (levelGap >= 3 && jobLevel === 0) {
      // Large gap but job has no seniority markers — ambiguous, not confirmed mismatch
  ...
  if (!scoredJobs.length) {
    ...
    for (let j = 0; j < jobs.length; j++) {
      ...
      if (fbGap >= 3 && fbJobLevel > 0) local.score = Math.round(local.score * 0.45);
      else if (fbGap >= 3 && fbJobLevel === 0) local.score = Math.round(local.score * 0.70);
  ...
  for (let i = 0; i < topCandidates.length; i += LLM_BATCH_SIZE) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      allResults.push({
        ...batch[j],
        match_score: combined || batch[j]._localScore || 0, // Fallback safely
        _llmScore: llmScore,
      });
    }
