...
import { logMatch } from './debug/match-logger.js';
...
const MATCH_THRESHOLD = 25;  // Let jobs scoring in the high 20s pass to LLM secondary validation
...
Return ONLY: [score1, score2, ...]`;
  ...
  try {
    ...
    logError('LLM scoring failed:', e.message);
    // Try fallback model
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
      ...
      logError('Fallback model also failed:', e2.message);
...
export async function matchJobs(jobs, profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  // ---- Phase 0: Batch Semantic Embeddings ----
  onProgress?.('🧠 Pre-computing semantic embeddings...');
  try {
    const { getEmbedding, getBatchEmbeddings, buildRoleText, buildJobText } = await import('./embeddings.js');
    const roleText = buildRoleText(profile);
    const roleEmb = await getEmbedding(roleText);
    if (roleEmb) {
      const jobTexts = jobs.map(j => buildJobText(j));
      const jobEmbs = await getBatchEmbeddings(jobTexts);
      
      for (let i = 0; i < jobs.length; i++) {
        jobs[i].__precomputedJobEmb = jobEmbs[i];
        jobs[i].__precomputedRoleEmb = roleEmb;
      }
    }
  } catch (err) {
     logError('Batch Embeddings failed:', err.message);
  }
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    if (local.score < MATCH_THRESHOLD) {
      filtered.lowScore++;
      // Log discarded jobs so we can audit what the engine is skipping
      _logPromises.push(logMatch({
        stage: 'list_score',
        profile,
        job,
        pandaScore: local.score,
        pandaBreakdown: local,
        llmScore: null,
        aiVerdict: null,
        combinedScore: local.score,
        notes: `discarded:below_threshold(${MATCH_THRESHOLD})`,
      }));
      continue;
    }
  ...
  for (let i = 0; i < topCandidates.length; i += LLM_BATCH_SIZE) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      const finalMatchScore = combined || Math.round(batch[j]._localScore) || 0;

      // Debug logging — capture every scored job for offline analysis
      _logPromises.push(logMatch({
        stage: 'list_score',
        profile,
        job: batch[j],
        pandaScore: batch[j]._localScore,
        pandaBreakdown: batch[j]._localDetail,
        llmScore,
        aiVerdict: null,
        combinedScore: finalMatchScore,
        notes: hasSeniorityMismatch ? `seniority_gap=${jobSeniorityGap}` : null,
      }));

      allResults.push({
        ...batch[j],
        match_score: finalMatchScore, // Fallback safely (round float)
        _llmScore: llmScore,
      });
    }
  ...
  // Flush all log writes before returning — unawaited promises are killed on serverless exit
