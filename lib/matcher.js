...
function scoreLocally(job, primaryKw, secondaryKw, titleWords, candidateYears, candidateHeadline, countryAliases) {
  ...
  if (countryAliases && countryAliases.length > 0) {
    ...
    if (matchesUserRegion) {
      locationMultiplier = 1.15; // 15% buff for jobs in the user's selected region
    } else if (isRemote) {
      locationMultiplier = 1.0; // Remote is neutral — no buff, no penalty
    } else {
      // Job mentions a specific location that doesn't match the user's region
      // Check if it explicitly mentions a DIFFERENT country
      ...
      const mentionsForeignCountry = foreignCountries.some(fc => {
        // Only penalize if the foreign country is NOT one of the user's aliases
        return jobGeo.includes(fc) && !countryAliases.includes(fc);
      });

      if (mentionsForeignCountry) {
        locationMultiplier = 0.7; // 30% penalty for explicitly foreign jobs
      }
      // If no country is mentioned at all, stay neutral (1.0x)
    }
  }
