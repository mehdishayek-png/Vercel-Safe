...
import { TITLE_SYNONYMS } from './dictionaries/titles';
import { STEM_MAP } from './dictionaries/stems';
import { DOMAIN_TERMS } from './dictionaries/domains';
//
...
function scoreLocally(job, primaryKw, secondaryKw, titleWords, candidateYears, candidateHeadline, countryAliases, cityAliases) {
  ...
  const deterministicVariance = combined.length % 15;
  const baseScore = 25 + deterministicVariance; // Base variance of 0-14 (range: 25-39)
  let earnedPoints = 0; // Merits accumulated from keywords, etc.

  const matchedPrimary = [];
  const matchedSecondary = [];
  let titleMatchCount = 0;

  // 2. DENSITY DEBUFF & ROLE FAMILY ENFORCER (Title Synergy)
  const wordCount = combined.split(/\s+/).length || 1;
  const densityPenalty = Math.max(0.4, Math.min(1.0, 400 / wordCount));

  let isRoleFamilyMatch = false;
  let hasRecognizedRoleFamily = false;
  const safeHeadline = (candidateHeadline || '').trim();

  if (safeHeadline && title.includes(safeHeadline)) {
    isRoleFamilyMatch = true;
  }

  // Check TITLE_SYNONYMS to map role families
  if (safeHeadline) {
    for (const [baseTitle, synonyms] of Object.entries(TITLE_SYNONYMS)) {
      if (safeHeadline.includes(baseTitle)) {
        hasRecognizedRoleFamily = true;
        if (synonyms.some(syn => title.includes(syn))) {
          isRoleFamilyMatch = true;
          break;
        }
      }
  ...
  let titleSynergyBuff = 1.0;
  let titleSynergyPenalty = 1.0;

  if (isRoleFamilyMatch) {
    titleSynergyBuff = 1.3; // 30% buff to earned points for explicit role match
  } else if (hasRecognizedRoleFamily && !isRoleFamilyMatch) {
    titleSynergyPenalty = 0.4; // *Hard Counter*: Candidate's role is known, but job title doesn't map to it.
  }
  ...
  for (const kw of primaryKw) {
    if (combined.includes(kw)) {
      ...
      // Punish rapid-fire keyword stuffing harder
      const diminishingFactor = Math.max(0.05, 1.0 - (keywordHits * 0.18));

      // Continuous weighting: longer/more-specific keywords are worth more
      const kwScore = 3 + Math.min(kw.length * 1.2, 15); // Range: ~4.2 (short) to 18 (long)

      rawKeywordScore += (kwScore * diminishingFactor * densityPenalty);
    }
  }

  // Hard Cap: Max 40 points from raw keyword hits (raised from 25 to let skill-dense matches separate)
  earnedPoints += Math.min(rawKeywordScore, 40);

  // Core Stack Ratio (Armor Pen): If they hit 4+ skills, provide a synergy bonus, scaled by density
  if (keywordHits >= 4) {
    earnedPoints += (10 * densityPenalty); // Reduced from 15
  }
  ...
  if (titleMatchCount >= 2) earnedPoints += 5;
  else if (titleMatchCount === 1) earnedPoints += 2;
  ...
  let seniorityBuff = 1.0;
  let seniorityPenalty = 1.0;

  // Continuous seniority gradient based on year-gap distance
  // instead of discrete buckets, so a 5yr candidate vs 3yr candidate
  // get meaningfully different scores for the same senior role.
  const jobExpectedYears = isJuniorJob ? 1 : isSeniorJob ? 7 : 3;
  ...
  if (Math.abs(yearGap) <= 2) {
    ...
    seniorityBuff = 1.25 - (Math.abs(yearGap) * 0.06); // 1.25 (exact) → 1.13 (2yr off)
  } else if (yearGap < -2) {
    // Reaching UP: mild penalty that scales with gap (ambition is okay, not crushed)
    seniorityPenalty = Math.max(0.15, 1.0 - (Math.abs(yearGap) - 2) * 0.15);
    // 3yr gap → 0.85, 5yr gap → 0.55, 7yr gap → 0.25
  } else {
    // Reaching DOWN (overqualified): harsh, scales with gap
    seniorityPenalty = Math.max(0.05, 1.0 - (yearGap - 2) * 0.25);
    // 3yr over → 0.75, 4yr over → 0.50, 5yr over → 0.25, 6yr+ → 0.05
  }
  ...
  if (job.date_posted) {
    ...
    if (!isNaN(postedDate)) {
      ...
      // Day 0-2: 1.10 (fresh buff), then decays smoothly
      // Day 7 → ~0.91, Day 14 → ~0.68, Day 21 → ~0.51, Day 30 → ~0.35
      if (diffDays <= 2) recencyMultiplier = 1.10;
      else recencyMultiplier = Math.max(0.25, 1.10 * Math.exp(-0.04 * (diffDays - 2)));
    }
  }

  const jobGeo = `${title} ${summary} ${jobLocation}`;
  // Only check title + location for remote — descriptions mention "remote" in passing
  const titleLocationOnly = `${title} ${jobLocation}`;
  const isRemote = titleLocationOnly.includes('remote') || titleLocationOnly.includes('work from home') || titleLocationOnly.includes('wfh');
  ...
  if (countryAliases && countryAliases.length > 0) {
    ...
    if (cityAliases && cityAliases.length > 0) {
      if (!exactCityMatch && !isRemote) {
        ...
        if (jobLocation.includes(',')) {
          // It's structured, like "Palakkad, Kerala, India".
          // If our specific city wasn't matched above, then this is definitely wrong.
          ...
          if (majorCities.some(c => jobGeo.includes(c))) {
            ...
            // It doesn't have a comma, isn't a known major city, and doesn't match the user region?
            // Safer to call it explicitly wrong.
            explicitlyWrongCity = true;
          }
        }
      }
    ...
    if (exactCityMatch) {
      ...
      locationMultiplier = 0.01; // INSTANT KILL for explicitly wrong country
    } else if (explicitlyWrongCity) {
      locationMultiplier = 0.02; // Near-kill (fatal) for explicitly wrong city within same country. A 100 match becomes a 2.
    } else if (vagueCountryMatch) {
      locationMultiplier = 0.10; // Massive penalty — vague country, user wanted a specific city. A 100 match becomes a 10.
  ...
  let locationBuff = 1.0;
  let locationPenalty = 1.0;

  if (locationMultiplier > 1.0) {
    locationBuff = locationMultiplier;
  } else if (locationMultiplier < 1.0) {
    locationPenalty = locationMultiplier;
  }

  // APPLY ALL MULTIPLIERS
  // 1. Buffs multiply only the earned points (merit)
  const finalEarned = earnedPoints * titleSynergyBuff * seniorityBuff * locationBuff;

  // 2. Penalties multiply the TOTAL score (base + finalEarned) to drag garbage matches down heavily
  let score = (baseScore + finalEarned) * titleSynergyPenalty * seniorityPenalty * recencyMultiplier * languageMultiplier * locationPenalty;

  // 3. HARD CAPS
  if (titleSynergyPenalty <= 0.4) {
    score = Math.min(score, 50); // Mismatched career track CANNOT score higher than 50
  }
  if (seniorityPenalty <= 0.2) {
    score = Math.min(score, 30); // Hard seniority blockers cannot score higher than 30
  }

  return {
    score: Math.min(score, 100), // Preserve float — round only once at final combined stage
...
async function llmBatchScore(batch, profile, candidateYears, apiKey, preferences) {
  ...
  // Enriched context from search strategy (kept brief to avoid confusing the model)
  const strategy = profile.search_strategy || {};
  const competitiveEdge = strategy.competitive_edge || '';
  const targetCompanies = (strategy.target_company_types || []).join(', ');

  const jobsText = batch.map((j, i) =>
    `JOB ${i + 1}: ${j.title || '?'} @ ${j.company || '?'} | ${j.location || '?'} | ${(j.summary || '').slice(0, 300)}`
  ).join('\n');

  const industryNote = industry ? `, Industry: ${industry}` : '';
  const edgeNote = competitiveEdge ? `, Edge: ${competitiveEdge.slice(0, 100)}` : '';
  const companyNote = targetCompanies ? `, Prefers: ${targetCompanies}` : '';
  const roleAnchor = preferences?.roleAnchor || '';
  const dominantPlatform = preferences?.dominantPlatform || '';
  const platformNote = dominantPlatform ? `, Core platform: ${dominantPlatform}` : '';
  const roleNote = roleAnchor ? `, Role anchor: ${roleAnchor}` : '';

  const prompt = `Score ${batch.length} jobs for this candidate. Return ONLY a JSON array of ${batch.length} integers 0-100.

Candidate: ${headline}, ${skills}, ~${candidateYears}yr exp${industryNote}${edgeNote}${companyNote}${platformNote}${roleNote}
Location preference: ${preferences?.location || 'Not specified'}

${jobsText}

Scoring: Skills 30%, Role fit 20%, Seniority 30%, Company 20%.
SENIORITY RULES (critical): If candidate has ${candidateYears}+ years exp, jobs with no "Senior"/"Lead"/"Principal" in the title are likely below their level — score ≤40. Junior/entry roles for senior candidates → score ≤20. Exact seniority match → no penalty.
...
Return ONLY: [score1, score2, ...]`;
  ...
  try {
    ...
    // Architect Gamma: Robust JSON array extraction
    ...
    try {
      ...
      const match = text.match(/\[[\s\S]*\]/);
      if (!match) throw new Error('No JSON array found in fallback LLM response');
      text = match[0];
...
export async function matchJobs(jobs, profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    // Architect Alpha: Async Yielding
    // Prevent event loop blocking when processing hundreds of jobs synchronously
    if (i > 0 && i % 25 === 0) {
    ...
    const local = await calculatePandaScore(job, profile, preferences, apiKeys);

    // ── POST-PANDA SENIORITY CORRECTION ──
    // Panda's 0.85x seniority penalty gets overwhelmed by location/roleFamily buffs.
    // Apply a harder correction based on the granular level gap (0-5 scale).
    const candidateLevel = candidateSeniorityLevel(profile.headline, candidateYears);
    const jobLevel = titleSeniorityLevel(title);
    const levelGap = candidateLevel - jobLevel; // Positive = overqualified

    if (levelGap >= 3) {
      // Massive overqualification (e.g. exec/senior applying to entry/open role)
      local.score = Math.round(local.score * 0.45);
      local.seniorityGap = levelGap;
    } else if (levelGap >= 2 && jobLevel > 0) {
      // Clear overqualification — job has explicit level markers below candidate
      local.score = Math.round(local.score * 0.55);
      local.seniorityGap = levelGap;
    } else if (levelGap >= 2 && jobLevel === 0) {
      // Job has no seniority markers — ambiguous. Moderate penalty for senior candidates.
      local.score = Math.round(local.score * 0.70);
      local.seniorityGap = levelGap;
    } else if (levelGap <= -3) {
      // Massively underqualified (entry applying to exec role)
      local.score = Math.round(local.score * 0.35);
      local.seniorityGap = levelGap;
    } else if (levelGap <= -2) {
      // Underqualified (mid applying to exec/principal role)
      local.score = Math.round(local.score * 0.50);
      local.seniorityGap = levelGap;
    } else {
      local.seniorityGap = levelGap;
    }
  ...
  if (!scoredJobs.length) {
    ...
    for (let j = 0; j < jobs.length; j++) {
      ...
      // Apply same post-panda seniority correction as main path
      const fbCandLevel = candidateSeniorityLevel(profile.headline, candidateYears);
      const fbJobLevel = titleSeniorityLevel(job.title || '');
      const fbGap = fbCandLevel - fbJobLevel;
      if (fbGap >= 3) local.score = Math.round(local.score * 0.45);
      else if (fbGap >= 2 && fbJobLevel > 0) local.score = Math.round(local.score * 0.55);
      else if (fbGap >= 2 && fbJobLevel === 0) local.score = Math.round(local.score * 0.70);
      else if (fbGap <= -3) local.score = Math.round(local.score * 0.35);
      else if (fbGap <= -2) local.score = Math.round(local.score * 0.50);
      local.seniorityGap = fbGap;
  ...
  if (!apiKey) {
    ...
    const localOnly = topCandidates.map(j => ({
      ...j,
      match_score: Math.round(j._localScore), // Round once here for local-only path
    }));
    ...
    return diverse.slice(0, MAX_MATCHES).map(j => {
      ...
      clean.heuristic_breakdown = _localDetail;
      return clean;
    });
  }
  ...
  const allResults = [];
  let apiCalls = 0;
  const strategy = profile.search_strategy || null;
  ...
  for (let i = 0; i < topCandidates.length; i += LLM_BATCH_SIZE) {
    ...
    if (scores[0] === -1) {
      ...
      const localOnly = topCandidates.map(j => ({
        ...j,
        match_score: Math.round(j._localScore), // Round once here for fallback path
      }));
      const finalLocal = localOnly.filter(j => j.match_score >= 45);
      finalLocal.sort((a, b) => {
        if (b.match_score !== a.match_score) return b.match_score - a.match_score;
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      // Combined score: 55% local heuristic + 45% LLM verification
      // LLM now has enriched context (resume text, competitive edge, company quality).
      // Conservative 5% shift from original 60/40. High-confidence floor still protects heuristic winners.
      let combined = Math.round(localScore * 0.55 + llmScore * 0.45);

      // HIGH-CONFIDENCE FLOOR: If our heuristic scored ≥80, the LLM cannot
      // assassinate the match below 70 — UNLESS there's a seniority mismatch.
      // Seniority-mismatched jobs should NOT be protected, letting the LLM correct them.
      const jobSeniorityGap = batch[j]._localDetail?.seniorityGap || 0;
      const hasSeniorityMismatch = Math.abs(jobSeniorityGap) >= 2;
      if (localScore >= 80 && combined < 70 && !hasSeniorityMismatch) {
        combined = 70;
      }
      ...
      // Source quality boost — proportional so it differentiates high vs low scores
      // (a 70 becomes 78, a 50 becomes 56 — instead of both jumping by flat +12)
      if (source.includes('linkedin')) {
        combined = Math.min(Math.round(combined * 1.12), 100);
      } else if (PRIORITY_SOURCES.some(s => source.includes(s))) {
        combined = Math.min(Math.round(combined * 1.05), 100);
      }
      ...
      // DOMINANT PLATFORM BOOST
      // If the query planner identified a dominant platform (e.g. "Salesforce", "React", "Unity")
      // and the job mentions it, give a small synergy boost — these are career-defining tools.
      const domPlatform = (preferences?.dominantPlatform || '').toLowerCase();
      if (domPlatform && domPlatform.length > 2) {
        const jobText = `${batch[j].title} ${batch[j].summary}`.toLowerCase();
        if (jobText.includes(domPlatform)) {
          combined = Math.min(combined + 6, 100);
        }
      }

      // CANDIDATE-ALIGNED COMPANY BOOST
      // If the candidate expressed preference for certain company types and LLM agrees (≥70), synergy boost
      if (strategy && strategy.target_company_types && llmScore >= 70) {
        const companyLower = (batch[j].company || '').toLowerCase();
        const summaryLower = (batch[j].summary || '').slice(0, 500).toLowerCase();
        const companyText = `${companyLower} ${summaryLower}`;

        const COMPANY_TYPE_SIGNALS = {
          'fortune500': ['fortune 500', 'f500', 'global leader', 'multinational', 'nyse', 'nasdaq'],
          'vc_funded': ['series a', 'series b', 'series c', 'funded', 'backed by', 'venture', 'raised'],
          'big_tech': ['google', 'meta', 'apple', 'amazon', 'microsoft', 'netflix', 'nvidia'],
          'consulting': ['deloitte', 'mckinsey', 'bcg', 'bain', 'accenture', 'pwc', 'ey', 'kpmg'],
          'mid_market': ['established', 'profitable', 'market leader'],
        };

        for (const type of strategy.target_company_types) {
          const signals = COMPANY_TYPE_SIGNALS[type] || [];
          if (signals.some(sig => companyText.includes(sig))) {
      ...
      // Location boost: proportional ~8% if job mentions user's country/city
      if (userCountry && userCountry !== 'remote only') {
        ...
        if (countryAliases.some(a => jobText.includes(a))) {
          combined = Math.min(Math.round(combined * 1.08), 100);
        }
      }

      allResults.push({
        ...batch[j],
        match_score: combined || Math.round(batch[j]._localScore) || 0, // Fallback safely (round float)
        _llmScore: llmScore,
      });
    }
