...
async function llmBatchScore(batch, profile, candidateYears, apiKey, preferences) {
  ...
  // Enriched context from search strategy
  ...
  const resumeSnippet = (profile.resume_text || '').slice(0, 800);
  ...
  const edgeNote = competitiveEdge ? `\n- Competitive Edge: ${competitiveEdge}` : '';
  const companyNote = targetCompanies ? `\n- Preferred Company Types: ${targetCompanies}` : '';
  const resumeNote = resumeSnippet ? `\n- Resume Summary: ${resumeSnippet}` : '';
...
- Skills: ${skills}
- Experience: ~${candidateYears} years${edgeNote}${companyNote}${resumeNote}
...
- Role fit (25% weight)
- Seniority alignment (20% weight) — THIS IS CRITICAL:
...
- Company quality (20% weight):
  * Fortune 500, Big Tech (FAANG), well-funded startups, established brands: score higher
  * Unknown/suspicious companies, MLM, crypto scams: score lower
  * Weight this against the candidate's preferred company types if specified above
