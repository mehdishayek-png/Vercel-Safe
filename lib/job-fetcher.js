...
Respond ONLY with this JSON shape:
{
  ...
  try {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'qwen/qwen3.6-plus:free',   // free tier on OpenRouter
        temperature: 0,
...
async function sourceStream(name, fetchFn, onSourceComplete, allJobs, cacheKey) {
    ...
    if (cacheKey) {
        try {
            ...
            if (cached && Array.isArray(cached) && cached.length > 0) {
                log(`[CACHE_HIT] ${name}: ${cached.length} jobs from cache`);
                onSourceComplete(name, cached);
                allJobs.push(...cached);
                return cached;
            }
        } catch (err) {
            warn(`[CACHE] ${name} cache lookup failed: ${err.message}`);
        }
    }
    ...
    try {
        ...
        if (cacheKey && jobs.length > 0) {
            cacheJobs(cacheKey, jobs, 1800).catch(() => {});
        }
        return jobs;
    } catch {
        return [];
...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  if (rssSources.length > 0) {
    ...
    const rssPromises = rssSources.map(src =>
      sourceStream(src.name, src.makeFetcher(ctx), onSourceComplete, allJobs, buildSourceCacheKey(src.cacheId, activeQueries, location))
    );
