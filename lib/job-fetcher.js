...
// ---- SerpAPI REMOVED — replaced by free sources (Himalayas, Jobicy, Findwork) ----
// Original fetchSerpAPI function removed to eliminate $150/month dependency.
// Google Jobs data is now sourced via JSearch, Adzuna, ATS scraping, and free APIs.

// Legacy stub for backwards compatibility (returns empty results)
async function fetchSerpAPI() {
  log('[SERP] DISABLED — SerpAPI removed. Using free sources instead.');
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  // All sources in parallel — Adzuna + JSearch + LinkedIn + free APIs + ATS
  ...
  if (queries.length > 0) {
    ...
    onProgress?.(`Querying ${activeQueries.length} queries across JSearch + LinkedIn + Adzuna + Himalayas + Jobicy + Findwork...`);

    const [adzunaJobs, jsearchJobs, linkedinJobs, arbeitnowJobs, museJobs, himalayasJobs, jobicyJobs, findworkJobs] = await Promise.all([
    ...
    onProgress?.(`+${adzunaJobs.length} Adzuna, +${jsearchJobs.length} JSearch, +${linkedinJobs.length} LinkedIn, +${arbeitnowJobs.length} Arbeitnow, +${museJobs.length} The Muse, +${himalayasJobs.length} Himalayas, +${jobicyJobs.length} Jobicy, +${findworkJobs.length} Findwork.`);
...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    ...
    const results = await Promise.allSettled([adzunaPromise, jsearchPromise, linkedinPromise, arbeitnowPromise, musePromise, himalayasPromise, jobicyPromise, findworkPromise, atsPromise]);

    // Log any rejected sources
    const sourceNames = ['Adzuna', 'JSearch', 'LinkedIn', 'Arbeitnow', 'The Muse', 'Himalayas', 'Jobicy', 'Findwork', 'Direct (ATS)'];
    results.forEach((r, i) => {
    ...
    const adzunaJobs = results[0].status === 'fulfilled' ? results[0].value : [];
    const jsearchJobs = results[1].status === 'fulfilled' ? results[1].value : [];
    const linkedinJobs = results[2].status === 'fulfilled' ? results[2].value : [];
    const atsJobs = results[8].status === 'fulfilled' ? results[8].value : [];
