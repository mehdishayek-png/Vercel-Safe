...
TIER 3 — SKILL-ANCHORED (3 queries): Platform/tool + role
- Query 8: Their strongest platform + role (e.g. "Salesforce Consultant")
- Query 9: Second strongest skill + role (e.g. "Python Data Engineer")
- Query 10: Niche technical specialty (e.g. "Kubernetes DevOps Engineer")

TIER 4 — COMPANY-QUALITY TARGETING (2 queries): Surface jobs at quality employers
- Query 11: A query that targets quality companies (${companyTypes || 'fortune500, big_tech, vc_funded'})
  Examples: "enterprise product manager", "Series B startup engineer"
- Query 12: Niche + prestige signal
  Examples: "AI research engineer", "fintech lead engineer"
  Do NOT hardcode company names. Use industry/company-type terms that IMPLICITLY surface quality employers.
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    // All 3 sources in parallel — SerpAPI (12 queries) + Adzuna + JSearch (6 queries)
    const adzunaCountry = preferences.country || profile.country || 'US';
    const adzunaLocation = preferences.city || preferences.state || '';
    onProgress?.(`Querying Google Jobs (${queries.length} queries) + JSearch + Adzuna...`);

    const [serpResult, adzunaJobs, jsearchJobs] = await Promise.all([
    ...
    onProgress?.(`+${serpJobs.length} Google Jobs, +${adzunaJobs.length} Adzuna, +${jsearchJobs.length} JSearch.`);
