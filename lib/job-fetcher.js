...
import { getSourcesByPhase } from './sources/registry.js';
  ...
  for (const q of queries.slice(0, 5)) {
    ...
    try {
      ...
      for (const item of organic) {
        ...
        // Skip search results pages from job boards (not individual job listings).
        // Patterns: "1100+ Customer Experience Jobs in...", "67,210 Lead customer..."
        // These are aggregator search pages, not actual job postings.
        const titleLower = title.toLowerCase();
        const isSearchPage = /\d{2,}[\+,]?\s*(jobs?|openings?|vacancies|positions|results)/i.test(title)
          || /\bjobs?\s+in\s+/i.test(title) && /\d{2,}/.test(title)
          || titleLower.includes('salaries in') || titleLower.includes('salary in')
          || titleLower.includes('jobs available') || titleLower.includes('open jobs')
          || /^find\s+\d/i.test(title) || /^search\s+/i.test(title);
        if (isSearchPage) continue;

        // Skip results where the "company" is actually a job board domain
        const BOARD_HOSTNAMES = new Set([
        ...
        if (BOARD_HOSTNAMES.has(hostname) && (!company || BOARD_HOSTNAMES.has(company.toLowerCase().replace(/^www\./, '')))) continue;
...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  const { rss: rssSources, api: apiSources } = getSourcesByPhase({ isRemotePreferred, isMidasSearch });
