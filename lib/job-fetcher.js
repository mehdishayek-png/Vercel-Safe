...
async function fetchArbeitnow() {
  try {
    ...
    console.error('Arbeitnow failed:', e.message);
    return [];
  }
}
...
async function fetchTheMuse(queries) {
  ...
  for (const q of searchTerms) {
    try {
      ...
      if (!res.ok) {
        console.warn(`[THEMUSE] HTTP ${res.status} for "${q}"`);
        continue;
      }
      const data = await res.json();
      ...
      console.log(`[THEMUSE] "${q || 'all'}" → ${(data.results || []).length} jobs`);
      await new Promise(r => setTimeout(r, 500));
    } catch (e) {
      console.error(`[THEMUSE] "${q}" failed:`, e.message);
    }
  }

  console.log(`[THEMUSE] Total: ${allJobs.length} jobs`);
  return allJobs;
}
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403 && errText.includes('not subscribed')) {
          console.warn(`[LINKEDIN] Not subscribed to LinkedIn API — subscribe at rapidapi.com. Skipping all queries.`);
          return allJobs; // Exit early, don't spam remaining queries
        }
        if (res.status === 429) {
          console.warn(`[LINKEDIN] Rate limited — waiting and skipping remaining queries`);
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    if (!isRemotePreferred && paidJobCount < 10) {
      ...
      console.log(`[RSS_FALLBACK] Paid sources returned ${paidJobCount} jobs, activating RSS fallback`);
