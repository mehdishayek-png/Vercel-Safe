...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    ...
    const serpPromise = fetchSerpAPI(queries, location, serpKey).then(result => {
    ...
    const results = await Promise.allSettled([serpPromise, adzunaPromise, jsearchPromise, linkedinPromise, arbeitnowPromise, musePromise]);

    // Log any rejected sources
    const sourceNames = ['Google Jobs', 'Adzuna', 'JSearch', 'LinkedIn', 'Arbeitnow', 'The Muse'];
    results.forEach((r, i) => {
