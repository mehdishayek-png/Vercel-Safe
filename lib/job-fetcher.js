  ...
  for (const q of queries.slice(0, 6)) { // Increased from 3 to 6
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) if (process.env.NODE_ENV === 'development') console.error('[SERP_DIAGNOSTIC] ERROR: Invalid API key (HTTP 401)');
        if (res.status === 429) if (process.env.NODE_ENV === 'development') console.warn('[SERP_DIAGNOSTIC] WARNING: Rate limit exceeded (HTTP 429)');
        if (res.status === 403) if (process.env.NODE_ENV === 'development') console.error('[SERP_DIAGNOSTIC] ERROR: Access forbidden (HTTP 403)');
        if (res.status === 400) if (process.env.NODE_ENV === 'development') console.error('[SERP_DIAGNOSTIC] ERROR: Bad request - check query/location format (HTTP 400)');
        continue;
      }
...
export function buildQueries(profile, preferences = {}) {
  ...
  // Priority 2: Pure Tech Core Clusters (Extract short acronyms without spaces e.g., SSO, SAML, AWS)
  const coreTechSkills = skills.filter(s => s.length <= 5 && !s.includes(' '));
  if (coreTechSkills.length >= 2) {
    const techCluster = coreTechSkills.slice(0, 3).join(' ');
    queries.push(`${techCluster} ${industry || 'tech'} jobs`);
    if (location) queries.push(`${techCluster} ${location}`);
  }
