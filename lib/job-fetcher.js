...
export function buildQueries(profile, preferences = {}) {
  ...
  const searchKeywords = profile.search_keywords && profile.search_keywords.length > 0
  ...
  if (industry && headline) {
    ...
    if (location) queries.push(`${industry} ${headline} ${location}`);
  }

  // Priority 3: Atomic keyword queries (from normalizer)
  // Short keywords like "SaaS", "AI", "CX", "b2b" make MUCH better queries
  // than raw multi-word skills like "technical troubleshooting"
  const shortKeywords = searchKeywords.filter(k => k.length <= 8 && !k.includes(' '));
  for (const kw of shortKeywords.slice(0, 4)) {
    if (industry) {
      queries.push(`${kw} ${industry} jobs`);
    } else {
      queries.push(`${kw} ${headline || 'jobs'}`);
    }
  }
  ...
  // Priority 5: Compound keyword+headline queries for remaining keywords
  for (const kw of searchKeywords.slice(0, 6)) {
  ...
  return { queries: unique.slice(0, 6), location, expandedQueries: unique.slice(0, 10), isRemotePreferred: isRemote };
}
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  const { queries, location, isRemotePreferred } = buildQueries(profile, preferences);
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    if (isSuperSearch) {
      ...
      const { expandedQueries } = buildQueries(profile, preferences);
