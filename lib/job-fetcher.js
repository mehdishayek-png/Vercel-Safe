...
async function fetchSerper(queries, location, apiKey) {
  ...
  for (const q of queries.slice(0, 5)) {
    const queryText = typeof q === 'string' ? q : (q.q || q);
    try {
      // Use Google Jobs API — returns structured job listings, not web search results.
      // This eliminates aggregator search pages (AmbitionBox "1100+ jobs", Glassdoor "67,210 jobs", etc.)
      const body = {
      ...
      // Google Jobs API returns structured job objects
      ...
      for (const item of jobs) {
        ...
        seen.add(dedupKey);
        if (link) { if (seen.has(link)) continue; seen.add(link); }
