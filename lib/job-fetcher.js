...
// Google domain mapping for geo-targeted results
const COUNTRY_GOOGLE_DOMAINS = {
  in: 'google.co.in', us: 'google.com', uk: 'google.co.uk', gb: 'google.co.uk',
  ca: 'google.ca', au: 'google.com.au', de: 'google.de', fr: 'google.fr',
  sg: 'google.com.sg', ae: 'google.ae', nl: 'google.nl', ie: 'google.ie',
  br: 'google.com.br', mx: 'google.com.mx', za: 'google.co.za', jp: 'google.co.jp',
  il: 'google.co.il', se: 'google.se', it: 'google.it', es: 'google.es',
};

// Canonical location mapping (SerpAPI expects full "City, State, Country" strings)
const CANONICAL_LOCATIONS = {
  ...
  'mumbai': 'Mumbai, Maharashtra, India', 'mumbai, india': 'Mumbai, Maharashtra, India',
  'delhi': 'Delhi, India', 'new delhi': 'Delhi, India', 'delhi, india': 'Delhi, India',
  'hyderabad': 'Hyderabad, Telangana, India', 'hyderabad, india': 'Hyderabad, Telangana, India',
  'pune': 'Pune, Maharashtra, India', 'pune, india': 'Pune, Maharashtra, India',
  'chennai': 'Chennai, Tamil Nadu, India', 'chennai, india': 'Chennai, Tamil Nadu, India',
  'kolkata': 'Kolkata, West Bengal, India', 'kolkata, india': 'Kolkata, West Bengal, India',
  'gurgaon': 'Gurugram, Haryana, India', 'gurugram': 'Gurugram, Haryana, India',
  'noida': 'Noida, Uttar Pradesh, India', 'kochi': 'Kochi, Kerala, India',
  'ahmedabad': 'Ahmedabad, Gujarat, India', 'india': 'India',
  'new york': 'New York, New York, United States', 'san francisco': 'San Francisco, California, United States',
  'sf': 'San Francisco, California, United States', 'seattle': 'Seattle, Washington, United States',
  'chicago': 'Chicago, Illinois, United States', 'austin': 'Austin, Texas, United States',
  'boston': 'Boston, Massachusetts, United States', 'los angeles': 'Los Angeles, California, United States',
  'denver': 'Denver, Colorado, United States', 'atlanta': 'Atlanta, Georgia, United States',
...
function normalizeSerpLocation(location) {
  if (!location) return null;
  const key = location.toLowerCase().trim();
  return CANONICAL_LOCATIONS[key] || location.replace(/\s+(Urban|Rural|District)/gi, '').trim();
}
...
async function fetchSerpAPI(queries, location, apiKey, { nextPageTokens = null, countryCode = null } = {}) {
  ...
  const allJobs = [];
  const seen = new Set();           // Primary dedup by job_id
  const seenTitleCompany = new Set(); // Fallback dedup by title+company
  let successfulQueries = 0;
  let failedQueries = 0;
  const collectedNextTokens = [];

  // Resolve google_domain from country code for better geo-targeting
  const gl = (countryCode || '').toLowerCase();
  const googleDomain = COUNTRY_GOOGLE_DOMAINS[gl] || 'google.com';
  const normalizedLocation = normalizeSerpLocation(location);

  const items = nextPageTokens ? nextPageTokens.slice(0, 7) : queries.slice(0, 10);
    ...
    try {
      ...
      if (!pageToken) {
        ...
        if (cached) {
          ...
          for (const job of cached) {
            const dedupKey = job.job_id || `${job.title}__${job.company}`.toLowerCase();
            if (!seen.has(dedupKey)) {
              seen.add(dedupKey);
              seenTitleCompany.add(`${job.title}__${job.company}`.toLowerCase());
              allJobs.push(job);
            }
          }
          successfulQueries++;
      ...
      // Build request URL using URLSearchParams for proper encoding
      let requestUrl;
      if (pageToken) {
        // SerpAPI requires q even with next_page_token
        requestUrl = `https://serpapi.com/search.json?engine=google_jobs&q=jobs&api_key=${apiKey}&next_page_token=${encodeURIComponent(pageToken)}`;
        ...
        requestUrl = `https://serpapi.com/search.json?${params.toString()}`;
        log(`[SERP] Fetching: "${queryText}" | ${normalizedLocation || 'global'} | gl=${gl || 'none'} | domain=${googleDomain}`);
      }
      ...
      // Collect next_page_token for pagination
      ...
      for (const job of (data.jobs_results || [])) {
        // Dual-layer dedup: job_id (Google's unique ID) + title/company fallback
        const jobId = job.job_id || null;
        const titleCompanyKey = `${job.title}__${job.company_name}`.toLowerCase();
        ...
        let applyUrl = pickBestApplyUrl(
          job.apply_options, job.company_name || '', job.title || '', jobId, queryText
        );
        ...
        const qualifications = highlights.find(h => h.title === 'Qualifications')?.items || [];
        const responsibilities = highlights.find(h => h.title === 'Responsibilities')?.items || [];
        ...
        const formattedJob = {
          ...
          job_id: jobId,
          is_remote: ext.work_from_home === true,
          schedule_type: ext.schedule_type || null,
      ...
      if (!pageToken && queryJobs.length > 0) {
        ...
        await cacheJobs(storeCacheKey, queryJobs, 21600);
      }
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    // Auto-broaden: if location-specific search returned few results
    if (location && !isRemotePreferred && serpJobs.length < 10) {
      const broaderQueries = queries.slice(0, 3);
      onProgress?.(`Few results (${serpJobs.length}) — broadening top queries at country level...`);
      log(`[SERP] Auto-broadening: ${serpJobs.length} jobs, retrying ${broaderQueries.length} queries at country level`);
      const countryLocation = normalizeSerpLocation(nonStreamCountryCode) || null;
      const { jobs: broaderJobs } = await fetchSerpAPI(broaderQueries, countryLocation, serpKey, { countryCode: nonStreamCountryCode });
...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    ...
    const serpPromise = fetchSerpAPI(queries, location, serpKey, { countryCode: serpCountryCode }).then(async (result) => {
      const { jobs, nextPageTokens } = result;
      onSourceComplete('Google Jobs', jobs);
      allJobs.push(...jobs);

      // Auto-paginate: fetch page 2 to double coverage (each page is only 10 results)
      if (nextPageTokens.length > 0) {
        onProgress?.(`Fetching page 2 for ${Math.min(nextPageTokens.length, 5)} top queries...`);
        const page2Result = await fetchSerpAPI([], null, serpKey, {
          nextPageTokens: nextPageTokens.slice(0, 5),
          countryCode: serpCountryCode,
        });
        serpTokens = page2Result.nextPageTokens || [];
        onSourceComplete('Google Jobs (page 2)', page2Result.jobs);
        allJobs.push(...page2Result.jobs);
        return [...jobs, ...page2Result.jobs];
      }
      serpTokens = [];
      return jobs;
    });
    ...
    if (location && !isRemotePreferred && serpJobs.length < 10) {
      const broaderQueries = queries.slice(0, 3);
      onProgress?.(`Few results (${serpJobs.length}) — broadening top queries at country level...`);
      log(`[SERP_STREAM] Auto-broadening: ${serpJobs.length} jobs, retrying ${broaderQueries.length} queries at country level`);
      const countryLocation = normalizeSerpLocation(serpCountryCode) || null;
      const { jobs: broaderJobs } = await fetchSerpAPI(broaderQueries, countryLocation, serpKey, { countryCode: serpCountryCode });
