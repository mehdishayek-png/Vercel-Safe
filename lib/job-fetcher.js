...
async function fetchSerpAPI(queries, location, apiKey) {
  ...
  // Reduced to 2 queries to conserve SerpAPI tokens (was 3)
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  // SerpAPI / JSearch - ONLY if free sources returned fewer than 50 jobs (cost optimization)
  const MIN_JOBS_BEFORE_PAID_API = 50;
  const shouldUsePaidAPIs = allJobs.length < MIN_JOBS_BEFORE_PAID_API;

  if (queries.length > 0 && shouldUsePaidAPIs) {
    console.log(`[INGESTION_ORCHESTRATOR] Free sources returned ${allJobs.length} jobs (< ${MIN_JOBS_BEFORE_PAID_API}). Activating SerpAPI.`);
