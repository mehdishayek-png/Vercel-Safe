...
async function fetchDataForSEO(queries, location) {
  ...
  // Build location_name in DataForSEO format. Their location DB expects names like:
  //   "Bengaluru,Karnataka,India" or "London,England,United Kingdom"
  // We pass through the user's location and let DataForSEO match it.
  let locationName = '';
  if (location) {
    const parts = location.split(',').map(s => s.trim()).filter(Boolean);
    locationName = parts.length >= 2 ? parts.join(',') : parts[0];
  }

  // Build task array — one task per query, max 5 to control cost (~$0.003 per scan at standard priority)
