...
async function fetchSerpAPI(queries, location, apiKey, startOffset = 0) {
  ...
  for (const q of queries.slice(0, 6)) { // Increased from 3 to 6
    try {
      ...
      let cached = null;
      const cacheKey = `serp_${encodeURIComponent((q.q || q).toLowerCase())}_${encodeURIComponent((normalizedLocation || 'global').toLowerCase())}${startOffset ? `_p${startOffset}` : ''}`;
      ...
      let requestUrl = `https://serpapi.com/search.json?engine=google_jobs&q=${encodeURIComponent(q.q || q)}&api_key=${apiKey}&num=10&start=${startOffset}&chips=date_posted:month`;
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    const serpJobs = await fetchSerpAPI(queries, location, serpKey, 0);
    allJobs.push(...serpJobs);
    onProgress?.(`+${serpJobs.length} from Google Jobs.`);
    if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] SerpAPI page 1:', serpJobs.length, 'jobs');

    if (isMidasSearch) {
      // Midas Search: 2x coverage — fetch page 2 (start=10) for each query
      onProgress?.(`✦ Midas Search: fetching page 2 for deeper coverage...`);
      const page2Jobs = await fetchSerpAPI(queries, location, serpKey, 10);
      allJobs.push(...page2Jobs);
      onProgress?.(`✦ +${page2Jobs.length} Midas Search results.`);
      if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] Midas Search page 2:', page2Jobs.length, 'jobs');
    }
