...
async function fetchSerpAPI(queries, location, apiKey) {
  ...
  if (process.env.NODE_ENV === 'development') console.log('[SERP_DIAGNOSTIC] Entry:', {
    serpTriggered: true,
    hasApiKey: !!apiKey,
  ...
  if (!apiKey) {
    if (process.env.NODE_ENV === 'development') console.warn('[SERP_DIAGNOSTIC] SKIPPED: API key not configured (SERP_API_KEY missing)');
    return [];
  }

  if (!queries.length) {
    if (process.env.NODE_ENV === 'development') console.warn('[SERP_DIAGNOSTIC] SKIPPED: No queries generated');
    return [];
  }
  ...
  for (const q of queries.slice(0, 6)) { // Increased from 3 to 6
    try {
      ...
      let cached = null;
      const cacheKey = `serp_${encodeURIComponent((q.q || q).toLowerCase())}_${encodeURIComponent((normalizedLocation || 'global').toLowerCase())}`;
      if (process.env.NODE_ENV !== 'test') { // Bypass cache during testing scripts
        cached = await getCachedJobs(cacheKey);
      }

      if (cached) {
        if (process.env.NODE_ENV === 'development') console.log(`[SERP_DIAGNOSTIC] Cache HIT ($0) - Using cached data for "${q.q || q}" (${normalizedLocation || 'global'}) `);
      ...
      if (process.env.NODE_ENV === 'development') console.log(`[SERP_DIAGNOSTIC] Cache MISS (!$) - Fetching live from SerpAPI...`);

      let requestUrl = `https://serpapi.com/search.json?engine=google_jobs&q=${encodeURIComponent(q.q || q)}&api_key=${apiKey}&num=10&chips=date_posted:month`;
      if (q.location || normalizedLocation) {
        requestUrl += `&location=${encodeURIComponent(q.location || normalizedLocation)}`;
      }
      if (process.env.NODE_ENV === 'development') console.log(`[SERP_DIAGNOSTIC] Query: "${q.q || q}" | Location: ${q.location || normalizedLocation || 'none'}`);

      const res = await fetch(requestUrl, {
        signal: AbortSignal.timeout(15000),
      });

      if (process.env.NODE_ENV === 'development') console.log(`[SERP_DIAGNOSTIC] Response: HTTP ${res.status} for query "${q.q || q}"`);
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (process.env.NODE_ENV === 'development') console.error(`[SERP_DIAGNOSTIC] HTTP ${res.status} Error Body:`, errorBody.slice(0, 500));
        } catch (e) {
          if (process.env.NODE_ENV === 'development') console.error(`[SERP_DIAGNOSTIC] Could not read error body`);
        }

        if (res.status === 401 && process.env.NODE_ENV === 'development') console.error('[SERP_DIAGNOSTIC] ERROR: Invalid API key (HTTP 401)');
        if (res.status === 429 && process.env.NODE_ENV === 'development') console.warn('[SERP_DIAGNOSTIC] WARNING: Rate limit exceeded (HTTP 429)');
        if (res.status === 403 && process.env.NODE_ENV === 'development') console.error('[SERP_DIAGNOSTIC] ERROR: Access forbidden (HTTP 403)');
        if (res.status === 400 && process.env.NODE_ENV === 'development') console.error('[SERP_DIAGNOSTIC] ERROR: Bad request - check query/location format (HTTP 400)');
        continue;
      }

      const data = await res.json();
      const jobsFound = (data.jobs_results || []).length;
      if (process.env.NODE_ENV === 'development') console.log(`[SERP_DIAGNOSTIC] Jobs found: ${jobsFound} for query "${q.q || q}"`);
      ...
      if (queryJobs.length > 0) {
        ...
        if (process.env.NODE_ENV === 'development') console.log(`[SERP_DIAGNOSTIC] Successfully cached ${queryJobs.length} live results for 24hrs.`);
      }
      ...
      failedQueries++;
      if (process.env.NODE_ENV === 'development') console.error(`[SERP_DIAGNOSTIC] ERROR: Query "${q.q || q}" failed:`, e.message);
    }
  }

  // DIAGNOSTIC LOG: Summary
  if (process.env.NODE_ENV === 'development') console.log('[SERP_DIAGNOSTIC] Summary:', {
...
async function planQueriesWithLLM(profile, apiKey) {
  ...
  const prompt = `You are a job search query optimizer for a resume-matching product.

Given this candidate profile, generate the 3 best Google Jobs search queries.

RULES:
- Query 1 MUST combine their primary platform/tool + their role title (e.g. "Salesforce Solution Architect")
- Query 2 adds their top technical differentiator to Query 1 (e.g. "Salesforce Solution Architect SOQL")  
- Query 3 is a slightly broader fallback (e.g. "Salesforce CRM Architect")
- Never generate generic queries like "Solution Architect" alone — always anchor to a platform/tool
...
Respond ONLY with this JSON shape:
{
  ...
  try {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'google/gemini-flash-1.5',   // fastest + cheapest on OpenRouter
        temperature: 0,
        max_tokens: 200,
...
export async function buildQueries(profile, preferences = {}, apiKey = null) {
  ...
  // search_keywords: atomic terms from the skill normalizer (generated at upload time)
  // Falls back to normalizing raw skills on-the-fly for backward compatibility
  const allKeywords = profile.search_keywords && profile.search_keywords.length > 0
    ? profile.search_keywords
    : normalizeSkillsForSearch(skills);

  // Pick top 5 most impactful skills for query generation
  // Everything else is noise that dilutes SerpAPI query quality
  ...
  if (llmPlan && llmPlan.queries.length > 0) {
    // LLM success — use its fused queries directly
    // Append location to each query if not remote
    const queries = llmPlan.queries.map(q =>
  ...
  // Each of the 5 best skills gets paired with the headline or industry
  ...
  if (headline && location) queries.push(`${headline} ${location}`);
  ...
  // Enforce max 8 words per query — SerpAPI performs best with short queries
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  const isSuperSearch = preferences.superSearch === true;
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] SerpAPI execution gate: queries.length =', queries.length);
    if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] SERP_API_KEY present:', !!serpKey);
    if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] Super Search:', isSuperSearch);

    if (isSuperSearch) {
      // Super Search: use multiple keys and expanded queries for 3x coverage
      const { expandedQueries } = await buildQueries(profile, preferences, openRouterKey);
      const serpKeys = [serpKey, process.env.SERP_API_KEY_2, process.env.SERP_API_KEY_3].filter(Boolean);
      onProgress?.(`⚡ Super Search: ${expandedQueries.length} queries × ${serpKeys.length} sources...`);

      const superSearchPromises = expandedQueries.map((q, i) => {
        const keyForQuery = serpKeys[i % serpKeys.length];
        return fetchSerpAPI([q], location, keyForQuery);
      });

      const superResults = await Promise.all(superSearchPromises);
      const superJobs = superResults.flat();
      allJobs.push(...superJobs);
      onProgress?.(`⚡ +${superJobs.length} Super Search results.`);
      if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] Super Search contributed:', superJobs.length, 'jobs');
    } else {
      ...
      if (process.env.NODE_ENV === 'development') console.log('[INGESTION_ORCHESTRATOR] SerpAPI contributed:', serpJobs.length, 'jobs');
    }
