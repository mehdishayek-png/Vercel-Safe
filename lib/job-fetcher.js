...
import { fetchLinkedInGuest } from './sources/linkedin-guest.js';
import { fetchApifyLinkedIn } from './sources/apify-linkedin.js';
import { fetchApifyDice as fetchApifyDiceActor, fetchApifyNaukri as fetchApifyNaukriActor } from './sources/apify-aggregators.js';
import { fetchWorkdayPublic } from './sources/workday-public.js';
import { getPremiumStartupSearchQueries } from './premium-portals.js';
import { resolveSearchLocation } from './search-location.js';
...
// Standardize raw location strings at ingest time so downstream filters
// match on canonical names instead of brittle substrings.
const CITY_ALIASES = {
  ...
  'nyc': 'New York', 'new york': 'New York', 'new york city': 'New York', 'manhattan': 'New York', 'brooklyn': 'New York',
...
function normalizeJobLocations(jobs) {
  for (const job of jobs) {
    const raw = (job.location || '').toLowerCase().trim();
    if (!raw) continue;

    const normalized = { raw: job.location };
    ...
    for (const [alias, canonical] of Object.entries(CITY_ALIASES)) {
      if (raw.includes(alias)) {
        normalized.city = canonical;
        break;
      }
    }

    // Extract country
    // Split by commas / pipes / dashes and check each segment
    const segments = raw.split(/[,|/\-–]/).map(s => s.trim()).filter(Boolean);
    for (const segment of segments) {
      const match = COUNTRY_ALIASES_GEO[segment];
      if (match) {
        normalized.country = match;
        break;
      }
    }
    // Fallback: check full string if segments didn't match
    if (!normalized.country) {
      for (const [alias, canonical] of Object.entries(COUNTRY_ALIASES_GEO)) {
        if (raw.includes(alias) && alias.length >= 3) { // Avoid false positives on 2-char codes in longer strings
  ...
  try {
    ...
    for (const slug of slugs.slice(0, 2)) { // Max 2 category pages
      try {
        ...
        // Recursive JSON Extractor: Immunity to UI/Routing structural changes
        const jobs = (function extractJobs(obj, requiredKeys) {
          let found = [];
          function search(node) {
            if (!node || typeof node !== 'object') return;
            if (Array.isArray(node)) {
              if (node.length > 0 && node[0] && typeof node[0] === 'object' && requiredKeys.every(k => k in node[0])) {
                found = found.concat(node.filter(n => n && typeof n === 'object' && requiredKeys.every(k => k in n)));
              }
          ...
          search(obj);
          // Deduplicate
          return found.filter((v, i, a) => a.findIndex(t => t.headline === v.headline && t.companyDetails?.name === v.companyDetails?.name) === i);
        })(data, ['headline', 'companyDetails']);
...
async function fetchHNWhoIsHiring() {
  try {
    ...
    for (let i = 0; i < commentIds.length; i += 15) {
      ...
      try {
        const { callFlash, parseJSON } = await import('./sonnet.js');
        const commentsText = validComments.map(c => `[ID: ${c.id}] (Time: ${c.time})\n${c.text.replace(/<[^>]+>/g, ' ').substring(0, 800)}`).join('\n\n---\n\n');
        
        const prompt = `You are a recruitment data extractor. Read the following Hacker News "Who is hiring?" comments. 
Each comment is separated by ---. Extract valid job listings. Many comments belong to someone looking for a job—IGNORE them.
Only extract posts where a company is clearly hiring. If a company is hiring, extract:
- id: the ID provided in brackets
- company: the company name
- title: the job title or a comma separated list of roles
- location: any location mentions (e.g., "SF, Remote, London")
- url: the website or apply link

Return a JSON object with a 'jobs' array containing the extracted objects. Return purely JSON, no markdown formatting.
Comments:\n${commentsText}`;

        const flashRes = await callFlash(prompt, { maxTokens: 4000, temperature: 0.1 });
        const parsed = parseJSON(flashRes);

        for (const job of (parsed.jobs || [])) {
          if (!job.company || !job.title || job.company === 'Unknown') continue;

          // Map back to comment
          const c = validComments.find(vc => String(vc.id) === String(job.id));
          if (!c) continue;

          const textBlock = c.text.replace(/<[^>]+>/g, ' ').replace(/&#x27;/g, "'").replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/\s+/g, ' ').trim();
          const urlFallback = textBlock.match(/https?:\/\/[^\s<"]+/);

          allJobs.push({
            title: job.title.substring(0, 120),
            company: job.company.substring(0, 80),
            summary: textBlock.substring(0, 5000),
            apply_url: job.url || urlFallback?.[0] || `https://news.ycombinator.com/item?id=${c.id}`,
            source: 'HN Hiring',
            date_posted: new Date(c.time * 1000).toISOString(),
            location: job.location || '',
            location_tags: extractLocationTags(textBlock.substring(0, 500)),
          });
        }
      } catch (err) {
        warn(`[HN_HIRING] LLM parsing failed: ${err.message}`);
      }
    }
...
async function fetchApna(queries, location) {
  try {
    ...
    // Recursive JSON Extractor: finds Apna job arrays regardless of payload structure
    const rawJobList = (function extractApnaJobs(obj) {
      let found = [];
      function search(node) {
        if (!node || typeof node !== 'object') return;
        if (Array.isArray(node)) {
          // Apna wraps jobs in { type: 3, data: { title, organization } }
          const unwrapped = node.filter(n => n && typeof n === 'object' && n.type === 3 && n.data).map(n => n.data);
          if (unwrapped.length > 0 && unwrapped[0].title) {
            found = found.concat(unwrapped);
            return; // Don't recurse into found arrays
          }
          // Also check for flat arrays with title + organization directly
          if (node.length > 0 && node[0] && typeof node[0] === 'object' && 'title' in node[0] && 'organization' in node[0]) {
            found = found.concat(node.filter(n => n && n.title));
            return;
          }
          for (const item of node) search(item);
        } else {
...
const FALLBACK_ENC_KEY = '99je848e92v0904k';
const FALLBACK_DEC_KEY = '390fnwr03n03ehn3';
...
function weekdayEncrypt(obj) {
  ...
  const keyToUse = process.env.WEEKDAY_ENCRYPT_KEY || FALLBACK_ENC_KEY;
  const { key, iv } = evpBytesToKey(keyToUse, salt);
...
function weekdayDecrypt(b64) {
  ...
  const keyToUse = process.env.WEEKDAY_DECRYPT_KEY || FALLBACK_DEC_KEY;
  const { key, iv } = evpBytesToKey(keyToUse, salt);
  ...
  try {
    ...
    warn('[CRITICAL_FAILURE] Weekday AES key rotation detected! Decryption yielded garbage bytes. Update ENV vars immediately!');
    throw new Error('Weekday decryption failed: key rotated');
  }
}

async function fetchWeekday(queries, location) {
  if (process.env.DISABLE_WEEKDAY_SCRAPER === 'true') {
    return [];
  }
...
// Parse country name from a full location string like "Bangalore, Karnataka, India"
function extractCountryFromLocation(locationStr) {
  if (!locationStr) return 'United States';
  const parts = locationStr.split(',').map(p => p.trim());
  const last = parts[parts.length - 1].toLowerCase();
  const COUNTRY_MAP = {
    'india': 'India', 'in': 'India',
    'us': 'United States', 'usa': 'United States', 'united states': 'United States',
    'uk': 'United Kingdom', 'united kingdom': 'United Kingdom', 'gb': 'United Kingdom', 'england': 'United Kingdom',
    'canada': 'Canada', 'ca': 'Canada',
    'australia': 'Australia', 'au': 'Australia',
    'singapore': 'Singapore', 'sg': 'Singapore',
    'germany': 'Germany', 'de': 'Germany',
    'uae': 'United Arab Emirates', 'dubai': 'United Arab Emirates', 'united arab emirates': 'United Arab Emirates',
    'netherlands': 'Netherlands', 'nl': 'Netherlands',
    'france': 'France', 'fr': 'France',
  };
  return COUNTRY_MAP[last] || parts[parts.length - 1] || 'United States';
}

// Get currency for a country
function currencyForCountry(country) {
  const MAP = {
    'India': 'INR', 'United Kingdom': 'GBP', 'Canada': 'CAD',
    'Australia': 'AUD', 'Singapore': 'SGD', 'Germany': 'EUR',
    'France': 'EUR', 'Netherlands': 'EUR',
  };
  return MAP[country] || 'USD';
}

// Extract city (first part before comma) from a location string
function extractCityFromLocation(locationStr) {
  if (!locationStr) return '';
  return locationStr.split(',')[0].trim();
}

// ---- Fetch Apify Naukri Scraper (India jobs) ----
async function fetchApifyNaukri(queries, location, apiKey) {
  ...
  // Extract city for Naukri location filter (e.g. "Bangalore, Karnataka, India" → "Bangalore")
  const city = extractCityFromLocation(location);
  const querySlug = encodeURIComponent(queryText.replace(/\s+/g, '-'));
  const citySlug = city ? encodeURIComponent(city.replace(/\s+/g, '-')) : '';

  try {
    const searchUrl = city
      ? `https://www.naukri.com/${querySlug}-jobs-in-${citySlug}?k=${encodeURIComponent(queryText)}&l=${encodeURIComponent(city)}`
      : `https://www.naukri.com/${querySlug}-jobs?k=${encodeURIComponent(queryText)}`;
    const input = {
      startUrls: [searchUrl],
      maxConcurrency: 5,
    };
...
async function fetchAdzuna(queries, location, countryCode, appId, appKey) {
  ...
  if (!ADZUNA_COUNTRIES.has(country)) {
    warn(`[ADZUNA] SKIPPED: unsupported or unresolved country "${countryCode || 'unknown'}"`);
    return [];
  }
 ...
 * Tries city map first, then country code. Unresolved locations are skipped;
 * silently searching a different country creates expensive false positives.
 */
...
async function fetchDataForSEO(queries, location, preferences) {
  ...
  if (!locationCode) {
    warn(`[DATAFORSEO] SKIPPED: location could not be resolved (${location || userCountry || 'empty'})`);
    return [];
  }
...
Respond ONLY with this JSON shape:
{
  ...
  try {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      signal: AbortSignal.timeout(15000),   // 15s — Gemini 2.5 Flash can be slow
    });
...
export async function buildQueries(profile, preferences = {}, apiKey = null) {
  ...
  // Build a location string from whatever fields are available
  const preferredLocation = preferences.location
    || (preferences.city ? `${preferences.city}, ${preferences.country || ''}`.trim().replace(/,\s*$/, '') : '')
    || profile.location
    || profile.country
    || '';
  ...
  // ── Step 2: Fallback — smart rule-based query generation ────────────────────────────────
  // Goal: generate 3-4 word queries that look like what a recruiter would post.
  // NEVER prepend skills to the full headline — "SSO Customer Experience Leader" is not a job title.
  log('[QUERY_PLANNER] Using advanced rule-based fallback');
  const queries = [];

  // Extract a short role anchor: first 3 words of headline, or the whole thing if short
  const headlineWords = headline.split(/\s+/);
  const roleAnchorShort = headlineWords.slice(0, 3).join(' '); // e.g. "Customer Experience Operations"

  // Priority 1: Short role anchor (3 words max — Google Jobs sweet spot)
  if (roleAnchorShort) queries.push(roleAnchorShort);

  // Priority 2: Role synonyms (Customer Experience ↔ Customer Success ↔ Client Success)
  const ROLE_SYNONYMS = {
    'customer experience': ['Customer Success Manager', 'Customer Operations Lead', 'CX Manager'],
    'customer success': ['Customer Experience Manager', 'Client Success Manager', 'CX Lead'],
    'product manager': ['Product Owner', 'Senior Product Manager', 'Program Manager'],
    'software engineer': ['Software Developer', 'Senior Software Engineer', 'Backend Engineer'],
    'data scientist': ['Data Analyst', 'ML Engineer', 'Data Science Lead'],
    'sales': ['Account Executive', 'Business Development Manager', 'Sales Manager'],
    'marketing': ['Growth Manager', 'Marketing Manager', 'Brand Manager'],
    'operations': ['Operations Manager', 'Operations Lead', 'Business Operations'],
  };
  const headlineLower = headline.toLowerCase();
  for (const [key, syns] of Object.entries(ROLE_SYNONYMS)) {
    if (headlineLower.includes(key)) {
      for (const s of syns.slice(0, 2)) queries.push(s);
      break;
    }
  }

  // Priority 3: role anchor + seniority signal from headline
  const hasSenior = /\b(senior|lead|principal|head|director|vp)\b/i.test(headline);
  if (hasSenior && roleAnchorShort && !queries.some(q => q.toLowerCase().includes('senior'))) {
    queries.push(`Senior ${headlineWords.slice(0, 2).join(' ')}`);
  }

  // Priority 4: Search terms from profile (LLM-generated variations, usually short)
  ...
  if (roleAnchorShort && isRemote) queries.push(`${roleAnchorShort} remote`);
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    ...
    const [serperJobs, adzunaJobs, jsearchJobs, linkedinJobs, arbeitnowJobs, museJobs, himalayasJobs, jobicyJobs, findworkJobs, usajobsJobs, wwrJobs, joobleJobs, reedJobs, devitJobs, linkedinSearchJobs, activeDbJobs, indeedJobs, bingJobs, naukriJobs, weekdayJobs, apnaJobs, instahyreJobs, cutshortJobs, hnJobs] = await Promise.all([
      ...
      fetchApifyNaukri(activeQueries, location),
  ...
  // Geocoding normalization — stamp canonical city/country on every job
  normalizeJobLocations(unique);
...
async function sourceStream(name, fetchFn, onSourceComplete, allJobs, cacheKey, sourceType = 'unknown') {
    ...
    if (cacheKey) {
        try {
            ...
            if (cached && Array.isArray(cached) && cached.length > 0) {
                ...
                await onSourceComplete?.(name, cached, {
                    status: 'success', sourceType, cacheHit: true,
                    latencyMs: Date.now() - start, rawCount: cached.length,
                });
                allJobs.push(...cached);
                return cached;
            }
    ...
    try {
        ...
        const partial = jobs.some((job) => job?._actor_partial === true);
        warn(`[SOURCE] ${name}: ${jobs.length} jobs (${ms}ms)`);
        await onSourceComplete?.(name, jobs, {
            status: jobs.length === 0 ? 'empty' : partial ? 'partial' : 'success', sourceType, cacheHit: false,
            latencyMs: ms, rawCount: jobs.length,
        });
        allJobs.push(...jobs);
        ...
        await onSourceComplete?.(name, [], {
            status: 'failed', sourceType, cacheHit: false, latencyMs: ms, rawCount: 0,
            errorCode: reason === 'TIMEOUT' ? 'timeout' : reason === 'CIRCUIT OPEN' ? 'circuit_open' : 'source_error',
        });
        return [];
    }
}
...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  let { queries, location, isRemotePreferred, roleAnchor, dominantPlatform, source } = await buildQueries(profile, preferences, openRouterKey);
  const locationResolution = resolveSearchLocation({
    location: location || preferences.location || profile.location,
    country: preferences.country || profile.country,
    city: preferences.city,
    state: preferences.state,
    remoteOnly: isRemotePreferred,
  });
  location = locationResolution.queryLocation;
  const normalizedPreferences = {
    ...preferences,
    location: locationResolution.raw || preferences.location || '',
    country: locationResolution.countryCode || preferences.country || profile.country || '',
    city: locationResolution.city || preferences.city || '',
    state: locationResolution.state || preferences.state || '',
    remoteOnly: locationResolution.isRemote,
  };
  Object.assign(preferences, normalizedPreferences);
  preferences = normalizedPreferences;
  
  if (preferences.premiumStartups) {
      const baseRole = roleAnchor || queries[0] || profile.headline || 'Engineer';
      queries = getPremiumStartupSearchQueries(baseRole);
      warn(`[QUERY_PLANNER] Switching to Premium Startups mode targeted at ${baseRole}`);
  }
  ...
  const originalOnSourceComplete = onSourceComplete;
  const globalSeenHashes = new Set();
  onSourceComplete = async (sourceName, fetchedJobs, sourceMeta = {}) => {
      if (!originalOnSourceComplete) return;
      const uniqueNewJobs = [];
      for (const job of fetchedJobs) {
          const rawCompany = (job.company || '').toLowerCase().replace(/[^a-z0-9]/g, '');
          const rawTitle = (job.title || '').toLowerCase().replace(/[^a-z0-9]/g, '');
          const loc = (job.location || '').toLowerCase().replace(/[^a-z0-9]/g, '');
          const hashKey = rawTitle + '__' + rawCompany + '__' + loc;
          if (!globalSeenHashes.has(hashKey)) {
              globalSeenHashes.add(hashKey);
              uniqueNewJobs.push(job);
          }
      }
      await originalOnSourceComplete(sourceName, uniqueNewJobs, {
          ...sourceMeta,
          rawCount: sourceMeta.rawCount ?? fetchedJobs.length,
          uniqueCount: uniqueNewJobs.length,
          duplicateCount: Math.max(0, fetchedJobs.length - uniqueNewJobs.length),
      });
  };
  ...
  const queryCount = isMidasSearch ? Math.min(queries.length, 12) : Math.min(queries.length, 7);
  const activeQueries = queries.slice(0, queryCount);
  const adzunaCountry = locationResolution.adzunaCountry;
  const adzunaLocation = locationResolution.city || '';
  ...
  const ctx = {
    queries, location, activeQueries, apiKeys, resolvedKeys, preferences, profile, locationResolution,
    ...
    _fetchBingJobs: fetchBingJobs, _fetchApifyDice: fetchApifyDiceActor, _fetchApifyNaukri: fetchApifyNaukriActor,
    _fetchJobSpy: fetchJobSpy,
    ...
    _fetchLinkedInGuest: fetchLinkedInGuest,
    _fetchApifyLinkedIn: fetchApifyLinkedIn,
    _fetchWorkdayPublic: fetchWorkdayPublic,
  ...
  const userCountry = locationResolution.countryCode || preferences.country || profile.country || '';
  const phases = getSourcesByPhase({ isRemotePreferred, isMidasSearch, userCountry });
  const isConfigured = (sourceConfig) => !sourceConfig.apiKeyEnvs?.length
    || sourceConfig.apiKeyEnvs.some((key) => apiKeys[key] || resolvedKeys[key] || process.env[key]);
  const rssSources = phases.rss.filter(isConfigured);
  const apiSources = phases.api.filter(isConfigured);
  const skippedUnconfigured = [...phases.rss, ...phases.api].filter((sourceConfig) => !isConfigured(sourceConfig));
  if (skippedUnconfigured.length > 0) {
    log(`[SOURCE_CONFIG] Skipped unconfigured sources: ${skippedUnconfigured.map((item) => item.name).join(', ')}`);
  }
  const totalSources = rssSources.length + apiSources.length;
  const skippedGeo = SOURCE_REGISTRY.filter(s => s.enabled).length - (phases.rss.length + phases.api.length);
  if (skippedGeo > 0) {
  ...
  if (rssSources.length > 0) {
    ...
    const rssPromises = rssSources.map(src =>
      sourceStream(src.name, src.makeFetcher(ctx), onSourceComplete, allJobs, forceRefresh ? null : buildSourceCacheKey(src.cacheId, activeQueries, location), src.type)
    );
  ...
  if (queries.length > 0) {
    ...
    if (apiJobCount < 15 && location) {
      ...
      const broaderAdzunaPromise = sourceStream('Adzuna (broadened)', () => fetchAdzuna(broaderQueries, '', adzunaCountry, resolvedKeys.ADZUNA_APP_ID, resolvedKeys.ADZUNA_APP_KEY), onSourceComplete, allJobs, null, 'api');
      const broaderJsearchPromise = sourceStream('JSearch (broadened)', () => fetchJSearch(broaderQueries, null, resolvedKeys.JSEARCH_KEY), onSourceComplete, allJobs, null, 'api');
    ...
    if (!isRemotePreferred && apiJobCount < 10) {
      ...
      const remoteOkFallback = sourceStream('RemoteOK (fallback)', () => fetchRSS(REMOTEOK_FEED, 'RemoteOK', 50), onSourceComplete, allJobs, null, 'rss');
      const remotiveFallback = sourceStream('Remotive (fallback)', () => fetchRemotive(), onSourceComplete, allJobs, null, 'rss');
    ...
    if (isMidasSearch) {
      onProgress?.('Midas Search: adding bonus RSS sources for deeper coverage...');
      const midasRemoteOkPromise = sourceStream('RemoteOK (Midas)', () => fetchRSS(REMOTEOK_FEED, 'RemoteOK', 100), onSourceComplete, allJobs, null, 'rss');
      const midasRemotivePromise = sourceStream('Remotive (Midas)', () => fetchRemotive(), onSourceComplete, allJobs, null, 'rss');
  ...
  return { jobs: unique, sources, queries, roleAnchor, dominantPlatform, source, totalRaw: allJobs.length, locationResolution };
}
