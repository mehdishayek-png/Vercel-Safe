...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    ...
    const [serperJobs, adzunaJobs, jsearchJobs, linkedinJobs, arbeitnowJobs, museJobs, himalayasJobs, jobicyJobs, findworkJobs, usajobsJobs, wwrJobs, joobleJobs, reedJobs, devitJobs, linkedinSearchJobs, activeDbJobs, indeedJobs, bingJobs, apifyJobs, naukriJobs] = await Promise.all([
...
export async function fetchAllJobsStreaming(profile, apiKeys = {}, onSourceComplete, onProgress, preferences = {}) {
  ...
  if (queries.length > 0) {
    ...
    const sourceNames = ['Serper', 'Adzuna', 'JSearch', 'LinkedIn', 'Arbeitnow', 'The Muse', 'Himalayas', 'Jobicy', 'Findwork', 'USAJOBS', 'WeWorkRemotely', 'Jooble', 'Reed', 'DevITjobs', 'LinkedIn (Fantastic)', 'ActiveJobsDB', 'Indeed', 'Bing Jobs', 'Apify', 'Naukri', 'Direct (ATS)'];
    results.forEach((r, i) => {
