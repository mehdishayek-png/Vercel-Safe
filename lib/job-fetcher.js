...
async function fetchSerpAPI(queries, location, apiKey) {
  ...
  for (const q of queries.slice(0, 6)) {
    try {
      // Normalize location for SerpAPI (it expects simple city/country format)
      let normalizedLocation = location;
      if (location) {
        // "Bangalore Urban, India" -> "Bangalore, India"
        normalizedLocation = location.replace(/\s+(Urban|Rural|District)/gi, '').trim();
      }
