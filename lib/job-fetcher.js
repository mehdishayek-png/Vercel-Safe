...
// ---- Fetch DataForSEO Google Jobs (real Google Jobs structured data, not web search) ----
// Uses Live mode (~$0.002/search) for user-triggered scans. Standard queue (~$0.0006) reserved
// for the future job-alert cron. $50 deposit lasts ~25K Live searches or ~83K Standard searches.
async function fetchDataForSEO(queries, location) {
  ...
  if (!authHeader) {
    log('[DATAFORSEO] SKIPPED: No DATAFORSEO_AUTH (or LOGIN+PASSWORD) configured');
    return [];
  }
  if (!queries.length) {
    log('[DATAFORSEO] SKIPPED: No queries');
    return [];
  }

  // Build location string in DataForSEO format: "City,Region,Country"
  // Accepts plain location like "Bengaluru, India" → "Bengaluru,Karnataka,India" via simple inference
  let locationName = '';
  if (location) {
  ...
  // Build task array — one task per query, max 5 to control cost
  const tasks = queries.slice(0, 5).map(q => {
    ...
    return {
      ...
      language_code: 'en',
      depth: 20,  // 20 results per query — sweet spot for cost vs coverage
    };
  }).filter(t => t.keyword);
  ...
  try {
    const res = await fetch('https://api.dataforseo.com/v3/serp/google/jobs/live/advanced', {
      method: 'POST',
      headers: {
      ...
      signal: AbortSignal.timeout(30000),  // DataForSEO Live can take 5-15s
    });

    if (!res.ok) {
      warn(`[DATAFORSEO] HTTP ${res.status}`);
      return [];
    }
    ...
    if (data.status_code !== 20000) {
      warn(`[DATAFORSEO] API error ${data.status_code}: ${data.status_message}`);
      return [];
    }
    ...
    for (const task of (data.tasks || [])) {
      if (task.status_code !== 20000) {
        warn(`[DATAFORSEO] Task error ${task.status_code}: ${task.status_message}`);
        continue;
      }
      totalCost += task.cost || 0;
      const queryText = task.data?.keyword || '';

      for (const result of (task.result || [])) {
        for (const item of (result.items || [])) {
          if (item.type !== 'google_jobs_search' && item.type !== 'jobs_element') continue;

          const url = item.url || item.apply_link || (item.apply_options?.[0]?.link) || '';
          const dedupKey = `${(item.company_name || '').toLowerCase()}::${(item.title || '').toLowerCase()}`;
          ...
          allJobs.push({
            ...
            summary: (item.description || '').slice(0, 5000),
            apply_url: url,
            source: 'Google Jobs',
            date_posted: item.job_posted || item.posting_time || '',
            salary: item.salary || '',
            location_tags: extractLocationTags(item.location || ''),
          });
        }
      }
      log(`[DATAFORSEO] "${queryText}" → ${allJobs.length} jobs total (cost so far: $${totalCost.toFixed(4)})`);
    }

    log(`[DATAFORSEO] Done: ${allJobs.length} jobs from ${tasks.length} queries (total cost: $${totalCost.toFixed(4)})`);
    return allJobs;
  } catch (e) {
    logError('[DATAFORSEO] Failed:', e.message);
    return [];
  }
}
