...
async function fetchSerpAPI(queries, location, apiKey, startOffset = 0) {
  // Critical diagnostics — always log in production so Vercel logs show what happened
  console.log('[SERP] Entry:', { hasKey: !!apiKey, queries: queries.length, location: location || 'global', offset: startOffset });
  ...
  for (const q of queries.slice(0, 6)) { // Increased from 3 to 6
    try {
      ...
      if (cached) {
        console.log(`[SERP] Cache HIT: "${q.q || q}" → ${cached.length} cached jobs`);
      ...
      console.log(`[SERP] Fetching: "${q.q || q}" | ${q.location || normalizedLocation || 'global'} | offset=${startOffset}`);
      ...
      if (!res.ok) {
        ...
        try {
          ...
          console.error(`[SERP] HTTP ${res.status} for "${q.q || q}":`, errorBody.slice(0, 300));
        } catch (e) {
          console.error(`[SERP] HTTP ${res.status} for "${q.q || q}" (no body)`);
        }
        continue;
      }
      ...
      console.log(`[SERP] "${q.q || q}" → ${jobsFound} jobs`);
      ...
      failedQueries++;
      console.error(`[SERP] Query "${q.q || q}" exception:`, e.message);
    }
  }

  console.log(`[SERP] Done: ${successfulQueries}/${queries.slice(0, 6).length} ok, ${failedQueries} failed, ${allJobs.length} jobs (offset=${startOffset})`);
