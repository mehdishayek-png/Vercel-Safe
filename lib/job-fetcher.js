...
Given this candidate profile, generate 6 search queries that MAXIMIZE relevant job results.

Google Jobs sweet spot: 3-4 word queries work best. 5+ words returns 0 results.

STRATEGY — mix role-specific and slightly broader:
- Query 1: Their exact job title (e.g. "Customer Success Manager", "Game Designer")
- Query 2: Role + industry/domain (e.g. "SaaS Implementation Engineer", "Mobile Game Designer")
- Query 3: Alternative title for the same role (e.g. "Client Success Manager", "Level Designer")
- Query 4: Role + seniority (e.g. "Senior Account Manager", "Lead Product Manager")
- Query 5: Adjacent role they could transition to (e.g. "Solutions Architect", "Product Manager")
- Query 6: Their strongest platform/skill + role (e.g. "Salesforce Consultant", "Unity Developer")

RULES:
- Aim for 3-4 words per query (2 is too vague, 5+ returns nothing)
- NEVER exceed 4 words — no compound phrases like "Unity Game Designer F2P Mobile"
- At least 3 queries must include their core role title or a close synonym
- Include DIFFERENT job title variations, not rephrased versions of the same query
...
export async function fetchAllJobs(profile, apiKeys = {}, onProgress, preferences = {}) {
  ...
  if (queries.length > 0 && shouldUsePaidAPIs) {
    ...
    // Page 1 — SerpAPI + Adzuna in parallel
    const adzunaCountry = preferences.country || profile.country || 'US';
    const adzunaLocation = preferences.city || preferences.state || '';
    onProgress?.(`Querying Google Jobs + Adzuna...`);

    const [serpResult, adzunaJobs] = await Promise.all([
    ...
    onProgress?.(`+${serpJobs.length} Google Jobs, +${adzunaJobs.length} Adzuna.`);

    // Auto-broaden: ONLY if location-specific search returned almost nothing (< 3 jobs)
    // Threshold kept low to avoid flooding irrelevant non-local results
