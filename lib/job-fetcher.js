...
export function buildQueries(profile, preferences = {}) {
  ...
  // Priority 1.5: Industry-Headline Compound Queries (Strategic Roles)
  // For domain-specific roles like "SaaS Customer Experience", generate compound queries
  // that surface strategic roles instead of generic basic ones.
  if (industry && headline) {
    queries.push(`${industry} ${headline}`);
    if (location) queries.push(`${industry} ${headline} ${location}`);
  }

  // Priority 2: Broad Tech Queries. The engine needs VOLUME so the Panda Matcher can filter them.
  // Google Jobs fails if the query is too specific like "sso saml okta jobs".
  const coreTechSkills = skills.filter(s => s.length <= 5 && !s.includes(' '));
  for (const tech of coreTechSkills.slice(0, 3)) {
    queries.push(`${tech} ${industry || 'technology'} jobs`);
  }

  // Priority 3: Precise Skill Combinations
  ...
  if (headline) {
    if (isRemote) queries.push(`${headline} remote`);
    if (location) queries.push(`${headline} ${location}`);
  }
  for (const skill of skills.slice(0, 4)) {
    if (queries.length < 15 && skill.split(' ').length <= 3) {
      if (skill.length <= 5 && !skill.includes(' ')) {
        // Pure tech skill fallback
        queries.push(`${skill} jobs`);
      } else {
        queries.push(`${skill} ${headline || 'jobs'}`);
      }
    }
  }
