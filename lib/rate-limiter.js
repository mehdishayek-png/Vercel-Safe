...
function getRateLimiter() {
    ...
    if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
        ...
        ratelimit = new Ratelimit({
            ...
            analytics: true,
            prefix: 'midasmatch:ratelimit',
        });

        log('[RATE_LIMIT] Using Upstash Redis (persistent)');
        return ratelimit;
    }

    warn('[RATE_LIMIT] No Upstash Redis configured — using in-memory fallback (resets on cold start)');
    return null;
}
