...
async function fetchPage(query, geoId, location, start) {
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 429) {
                console.warn('[LINKEDIN_GUEST] 429 Rate Limit Hit - Aborting source.');
        ...
        if (e.message === '429_RATE_LIMIT' || e.name === 'TimeoutError') throw e;
        return [];
    }
}
...
export async function fetchLinkedInGuest(queries, location, preferences = {}) {
    ...
    try {
        ...
        if (e.message === '429_RATE_LIMIT' || e.name === 'TimeoutError') {
             console.warn(`[LINKEDIN_GUEST] Short-circuiting remaining queries due to: ${e.message || e.name}`);
        } else {
             console.error('[LINKEDIN_GUEST] Outer loop error:', e);
        }
    }
