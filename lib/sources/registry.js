...
export const SOURCE_REGISTRY = [
    ...
    // PRIMARY: DataForSEO Google Jobs (real structured Google Jobs data, ~$0.006/scan)
    {
        ...
        apiKeyEnvs: ['DATAFORSEO_AUTH', 'DATAFORSEO_LOGIN'],
    ...
    // ---- Apify aggregators, split for regional routing and source attribution ----
    {
        name: 'Dice (Apify)', cacheId: 'apify-dice', type: 'api', enabled: true,
        remoteOnly: false, midasOnly: false, geo: 'us', querySlice: 1, needsLocation: true,
        apiKeyEnvs: ['APIFY_API_TOKEN', 'APIFY_API_KEY'],
        makeFetcher: (ctx) => () => ctx._fetchApifyDice(ctx.activeQueries.slice(0, 1), ctx.location),
    },
    {
        name: 'Naukri (Apify)', cacheId: 'apify-naukri', type: 'api', enabled: true,
    ...
    {
        name: 'Wellfound', cacheId: 'wellfound', type: 'api', enabled: false,
    ...
    {
        name: 'Foundit', cacheId: 'foundit', type: 'api', enabled: false,
        remoteOnly: false, midasOnly: false, geo: 'india', querySlice: 1, needsLocation: true,
...
export function getActiveSources({ isRemotePreferred = false, isMidasSearch = false, userCountry = '' } = {}) {
    ...
    return SOURCE_REGISTRY.filter(s => {
        ...
        // Geo filtering: an unresolved country must not activate every regional
        // source. Global sources have no geo tag; remote boards are explicit.
        if (s.geo) {
            if (s.geo === 'remote') {
                return isRemotePreferred || isMidasSearch;
            }
            if (!allowedGeos || !allowedGeos.has(s.geo)) return false;
        }
        return true;
    });
}
