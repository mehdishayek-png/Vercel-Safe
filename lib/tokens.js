...
const FREE_DEEP_SCANS = 100; // Paywall removed for today
...
function weeklySuperScanKey(userId) {
    const d = new Date();
    // Use ISO week logic or simple 7-day rolling window by week number
    const oneJan = new Date(d.getFullYear(), 0, 1);
    const numberOfDays = Math.floor((d - oneJan) / (24 * 60 * 60 * 1000));
    const weekNum = Math.ceil((d.getDay() + 1 + numberOfDays) / 7);
    return `user:${userId}:weekly_super_scans:${d.getFullYear()}_${weekNum}`;
}
...
/**
 * Get weekly super scan count
 */
export async function getWeeklySuperScanCount(userId) {
    if (!redis || !userId) return 0;
    try {
        const count = await redis.get(weeklySuperScanKey(userId));
        return parseInt(count || '0', 10);
    } catch (err) {
        return 0;
    }
...
/**
 * Increment weekly super scan
 */
export async function incrementWeeklySuperScan(userId) {
    if (!redis || !userId) return { success: false };
    try {
        const key = weeklySuperScanKey(userId);
        const count = await redis.incr(key);
        if (count === 1) {
...
export async function canScan(userId, superSearch = false) {
    ...
    // Super Search
    if (superSearch) {
        // Check for free weekly super search
        const weeklySuperCount = await getWeeklySuperScanCount(userId);
        if (weeklySuperCount < 1) { // 1 free super search per week
            return {
                allowed: true,
                isFree: true,
                freeRemaining: 0, // Since it's only 1, 0 remains after this use
                source: 'server',
                isSuperSearchFree: true // Flag to tell matcher to increment weekly super scan instead of daily scan
            };
        }
