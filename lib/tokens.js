...
export async function getTokenBalance(userId) {
    ...
    try {
        ...
        // Give new users 5 free starter tokens if their wallet has never been initialized
        if (balance === null) {
            let startingTokens = 0;
            try {
                const user = await currentUser();
                // If they logged in directly via email, they will have 0 external accounts (OAuth)
                if (user && (!user.externalAccounts || user.externalAccounts.length === 0)) {
                    startingTokens = 5;
                }
            } catch (e) {
                warn('Error verifying Clerk user for starter tokens:', e);
            }
            await redis.set(key, startingTokens);
            balance = String(startingTokens);
        }
        
        return { tokens: parseInt(balance, 10), source: 'server' };
    } catch (err) {
