...
export async function getPendingVests(userId) {
    ...
    try {
        const now = Math.floor(Date.now() / 1000);
        // ZRANGEBYSCORE with WITHSCORES — Upstash returns [{score, member}, ...]
        const pending = await redis.zrange(vestKey(userId), now + 1, '+inf', {
            byScore: true,
            withScores: true,
        });
        if (!pending || pending.length === 0) return [];
        return pending.map(({ member, score }) => {
            const match = String(member).match(/:(\d+)$/);
            const amount = match ? parseInt(match[1], 10) : 0;
            return { amount, releaseAt: new Date(Number(score) * 1000).toISOString() };
        }).filter(v => v.amount > 0);
    } catch (err) {
