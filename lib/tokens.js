...
/**
 * Get anonymous scan count by IP (daily, tracked via Redis)
 */
const ANONYMOUS_DAILY_SCANS = 3;

async function getAnonymousScanCount(ip) {
    if (!redis || !ip) return 0;
    try {
        const today = new Date().toISOString().slice(0, 10);
        const count = await redis.get(`anon_scans:${ip}:${today}`);
        return parseInt(count || '0', 10);
    } catch { return 0; }
}

export async function incrementAnonymousScan(ip) {
    if (!redis || !ip) return;
    try {
        const today = new Date().toISOString().slice(0, 10);
        const key = `anon_scans:${ip}:${today}`;
        const count = await redis.incr(key);
        if (count === 1) await redis.expire(key, 86400);
    } catch (err) {
        warn('Redis incrementAnonymousScan error:', err);
    }
}
 ...
 */
export async function canScan(userId, midasSearch = false, ip = null) {
    // Anonymous users: track by IP with daily limit
    if (!userId) {
        if (redis && ip) {
            const anonCount = await getAnonymousScanCount(ip);
            if (anonCount >= ANONYMOUS_DAILY_SCANS) {
                return {
                    allowed: false,
                    error: 'Free scan limit reached. Sign in to get 5 free scans per day.',
                    requiresAuth: true,
                    source: 'anonymous'
                };
            }
            return {
                allowed: true,
                isFree: true,
                freeRemaining: ANONYMOUS_DAILY_SCANS - anonCount - 1,
                source: 'anonymous',
                anonymousPass: true,
                trackAnonymous: true,
                anonymousIp: ip
        ...
        // Redis unavailable — allow with no tracking (beta fallback)
        return {
            allowed: true,
            isFree: true,
            freeRemaining: 3,
