...
export async function deductToken(userId, amount = 1) {
    // Token system disabled — all signed-in users have unlimited access.
    // Billing will be reintroduced later; the atomic Redis logic below is kept
    // intact (dead) so it can be restored by removing this early return.
...
export async function canScan(userId, midasSearch = false) {
    ...
    // Token system disabled — every signed-in user gets unlimited free access.
    // Sign-in is still required (the userId guard above). Billing returns later.
    return {
        allowed: true,
        isFree: true,
        tokenCost: 0,
