...
const SENIOR_REGEX = /\b(senior|sr|lead|principal|vp|director|head|architect)\b/;
const MANAGER_REGEX = /\b(manager|supervisor)\b/;
const INTERN_REGEX = /\b(intern|internship|fresher|trainee|junior|jr|entry|administrator|admin)\b/;
...
export const NICHE_TOOLS = new Set([
...
const ROLE_FAMILIES = {
    engineering: [
        // Core software
        'software engineer', 'software developer', 'programmer', 'sde', 'swe',
        'fullstack', 'full stack', 'frontend engineer', 'backend engineer',
        'web developer', 'mobile developer', 'ios developer', 'android developer',
        'embedded engineer', 'firmware engineer', 'systems engineer', 'systems programmer',
        // DevOps / Platform / Cloud / SRE
        'devops', 'devsecops', 'sre', 'site reliability', 'platform engineer',
        'cloud engineer', 'infrastructure engineer', 'release engineer', 'build engineer',
        // QA / Testing / Automation
        'qa engineer', 'qe ', 'test engineer', 'sdet', 'automation engineer',
        'test automation', 'quality engineer', 'automation tester', 'testing engineer',
        // Security Engineering
        'security engineer', 'application security', 'appsec', 'penetration tester', 'pentest',
        // Support Engineering
        'iam engineer', 'technical support', 'it support',
    ],
    civil_engineering: [
        'civil engineer', 'structural engineer', 'site engineer', 'project engineer',
        'geotechnical', 'construction engineer', 'architectural designer', 'site supervisor',
        'quantity surveyor', 'design engineer', 'building engineer',
    ],
    cx_support: [
        'customer experience', 'customer success', 'customer support', 'cx ',
        'csm', 'support specialist', 'customer care', 'client success',
        'customer operations', 'product support',
    ],
    sales: [
        'sales', 'account executive', 'business development', 'bdr', 'sdr',
        'revenue', 'lead generation', 'inside sales', 'outside sales', 'account manager',
    ],
    data: [
        // Data science / ML / AI
        'data scientist', 'ml engineer', 'ai engineer', 'machine learning',
        'research scientist', 'research engineer', 'nlp engineer', 'computer vision',
        'quantitative analyst', 'quant ',
        // Data engineering / analytics
        'data engineer', 'analytics engineer', 'data analyst', 'data specialist',
        'data architect', 'database administrator', 'dba ', 'etl developer',
        // BI
        'business intelligence', 'bi developer', 'bi analyst', 'bi engineer',
        // Spatial / domain data
        'spatial data', 'geospatial', 'gis ',
        // Generic analytics (word-boundary applied by detectRoleFamily for short keywords)
        'analytics',
    ],
    // gaming before design: 'game ' must fire before bare 'designer' for "Game Designer" titles
    gaming: [
        'game ', 'gaming', 'game designer', 'system designer', 'level designer',
        'narrative designer', 'f2p', 'live ops', 'liveops', 'game economy',
        'game ui', 'monetization designer', 'gameplay',
    ],
    ...
    product: [
        ...
        'associate product manager', 'apm ', 'chief of staff',
    ],
    // content/media defined before marketing so 'content producer'/'production manager'
    // takes precedence over the bare 'content' marketing keyword (iteration order matters).
    marketing: ['marketing', 'growth', 'seo', 'brand'],
    // operations: pure business/supply-chain ops.
    operations: [
        'operations manager', 'ops manager', 'operations lead', 'operations specialist',
        'operations analyst', 'supply chain', 'logistics', 'procurement',
        'project coordinator', 'program coordinator', 'delivery coordinator',
        'localization coordinator', 'post-production coordinator',
    ],
    it_infra: [
        'incident manager', 'incident management', 'itil', 'service level',
        'change manager', 'problem manager', 'it infrastructure', 'network engineer',
        'system administrator', 'sysadmin', 'infrastructure', 'it manager',
        'cloud architect', 'network administrator',
    ],
    process: [
        'lean', 'six sigma', 'process excellence', 'process improvement',
        'quality manager', 'quality assurance manager', 'continuous improvement',
    ],
    finance: [
        'accountant', 'auditor', 'controller', 'bookkeeper',
        'financial analyst', 'finance analyst', 'investment banking', 'investment banker',
        'portfolio manager', 'treasury', 'fp&a', 'financial planning',
        'risk analyst', 'credit analyst', 'equity research', 'fund manager',
        'chartered accountant', 'ca ', 'cfa ', 'cpa ',
        'finance manager', 'finance director', 'finance business partner',
    ],
    ...
    devrel: [
        ...
        'community manager', 'technical evangelist', 'community engineer',
    ],
    content: [
        'content strategist', 'content creator', 'copywriter', 'content manager',
        'technical writer', 'documentation engineer', 'content lead', 'content specialist',
        'content writer', 'content producer', 'video producer', 'media producer',
        'post-production', 'film producer', 'showrunner', 'assistant director',
        'cinematographer', 'media operations',
        'video editor', 'video production', 'videographer', 'production assistant',
        'production coordinator', 'motion graphics', 'motion designer', 'animator',
        'vfx artist', 'colorist', 'color grading', 'sound designer', 'audio engineer',
        'podcast producer', 'broadcast',
        'photographer', 'photo editor', 'retoucher', 'creative producer', 'studio manager',
        // Kept separate from operations to avoid 'production manager' ambiguity
        'production manager',
    ],
    solutions_architecture: [
        'solutions architect', 'solution engineer', 'presales', 'pre-sales',
        'sales engineer', 'implementation engineer', 'solutions engineer',
        'professional services', 'implementation consultant',
        'technical account manager', 'tam ',
    ],
    product_marketing: [
        'product marketing', 'pmm ', 'go-to-market', 'gtm manager',
        'product marketer', 'demand generation', 'field marketing',
    ],
    legal_compliance: [
        'legal counsel', 'general counsel', 'paralegal', 'compliance officer',
        'data protection', 'privacy officer', 'contract manager', 'legal ops',
        'legal officer', 'advocate', 'attorney', 'lawyer', 'in-house counsel',
        'company secretary', 'corporate secretary', 'legal manager', 'legal executive',
        'legal advisor', 'solicitor',
    ],
    consulting: [
        // Strategy & Advisory — specific phrases only, no generic single words
        'strategy consultant', 'management consultant', 'deal advisory',
        'transaction advisory', 'due diligence', 'valuation analyst',
        'corporate finance', 'restructuring', 'engagement manager',
        // Generic 'consultant' and 'advisory' kept but word-boundary matched (≤8 chars handled by detectRoleFamily)
        'consultant', 'advisory',
        // Removed: 'business analyst' — too generic, fires for data/ops/tech BA roles
    ],
};
    ...
    for (const [cluster, keywords] of Object.entries(DOMAIN_CLUSTERS)) {
        ...
        for (const kw of keywords) {
            ...
            if (kwMatches) {
                ...
                const actualThreshold = cluster === 'salesforce_crm' ? 1.0 : weightThreshold;
                if (weightedScore >= actualThreshold) {
                    matched.add(cluster);
                    break;
                }
            }
...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    const SENIORITY_TO_YEARS = { intern: 0, entry: 1, administrator: 1, admin: 1, junior: 2, mid: 4, senior: 8, lead: 8, principal: 10, staff: 10, director: 12, vp: 15, 'c-level': 18 };
    ...
    const baseLocation = preferences.location || '';
    let userCity = normalize(preferences.city);
    let userState = normalize(preferences.state);
    let userCountry = normalize(preferences.country);
    
    if (!userCity && !userCountry && baseLocation) {
        if (baseLocation.includes(',')) {
            const parts = baseLocation.split(',').map(s => s.trim());
            userCity = normalize(parts[0]);
            const p2 = normalize(parts[1]);
            if (p2.length === 2 && p2 !== 'uk') {
                userState = p2;
        ...
        } else if (!['remote', 'hybrid', 'worldwide'].includes(baseLocation.toLowerCase())) {
            userCountry = normalize(baseLocation);
        }
    }
    
    const userLocation = normalize(baseLocation);
    ...
    const candidateFamily = profile._llmFamily || detectRoleFamily(targetTitle);
    const jobFamily = detectRoleFamily(title);

    if (candidateFamily && jobFamily) {
        if (candidateFamily === jobFamily) {
            roleFamilyMultiplier = SCORING_CONFIG.roleFamily.sameFamily;
        } else if (profile._llmAntiFamilies?.includes(jobFamily)) {
            roleFamilyMultiplier = 0.2;
        } else {
    ...
    // 12. CLOSED-LOOP PERSONALIZATION (RLHF)
    let feedbackMultiplier = 1.0;
    if (preferences.feedbackHistory && preferences.feedbackHistory.length > 0) {
        const companyFeedback = preferences.feedbackHistory.filter((fb) => normalize(fb.company) === company);
        const positive = companyFeedback.filter((fb) => fb.action === 'save' || fb.action === 'apply');
        const negative = companyFeedback.filter((fb) => fb.action === 'dismiss' || fb.action === 'skip');
        const signal = positive.length - negative.length;

        // Require repeated behavior for material changes. A single application is
        // a weak positive; repeated dismissals are a stronger negative signal.
        if (signal >= 2) {
            feedbackMultiplier = 1.05;
        } else if (positive.some((fb) => fb.action === 'apply')) {
            feedbackMultiplier = 1.03;
        } else if (signal <= -2) {
            feedbackMultiplier = 0.85;
        } else if (negative.length === 1) {
            feedbackMultiplier = 0.92;
        }
    }
    ...
    if (domainResult.reason === 'mismatch') {
        ...
        const skillOverride = matchedCount >= 3;
        const domainMismatchCap = sameFamilyOverride
            ? 80  // same-family CX title overrides domain cluster noise
            : skillOverride
            ? 65  // 3+ skill hits override domain cluster noise
            : isExploreMode ? SCORING_CONFIG.caps.domainMismatchCapExplore : SCORING_CONFIG.caps.domainMismatchCap;
        
        if (sameFamilyOverride || skillOverride) {
            // Apply a dynamic softening to the domain multiplier instead of recalculating 
            // the whole score again, we just boost the score by the difference in multiplier
            finalScore *= (0.85 / domainMultiplier);
        }
    ...
    if (!isExploreMode) {
        ...
        // Strong Technical Fit Override: if they match 3+ niche tools verbatim,
        // skills override the semantic title boundary limitation.
        const strongSkillMatch = matchedCount >= 3;
        
        if (roleFamilyMultiplier <= SCORING_CONFIG.roleFamily.crossFamily) {
            // Give 65 threshold to let strict skill matches rise into the "Good" tier
            const effectiveCap = strongSkillMatch ? 65 : (domainOverlaps ? 55 : SCORING_CONFIG.caps.crossFamilyCap);
            finalScore = Math.min(finalScore, effectiveCap);
            // Soften the math penalty since the hard-skills confirm the mapping
            if (strongSkillMatch) {
                finalScore *= (0.85 / roleFamilyMultiplier); // dynamically override from 0.40x to 0.85x
            }
        } else if (roleFamilyMultiplier <= SCORING_CONFIG.roleFamily.unclassifiable) {
            const effectiveCap = strongSkillMatch ? 65 : (domainOverlaps ? 50 : SCORING_CONFIG.caps.unclassifiedCap);
            finalScore = Math.min(finalScore, effectiveCap);
            if (strongSkillMatch) {
                 finalScore *= (0.85 / roleFamilyMultiplier); // dynamically override from 0.60x to 0.85x
            }
        }
    } else {
    ...
    return {
        ...
        multipliers: {
            ...
            feedback: feedbackMultiplier.toFixed(2),
        },
