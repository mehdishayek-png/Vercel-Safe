...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    if (preferences.feedbackHistory && preferences.feedbackHistory.length > 0) {
        for (const fb of preferences.feedbackHistory) {
            const fbCompany = normalize((fb.jobKey || '').split(':')[0] || '');
            if (fbCompany && fbCompany === company) {
                if (fb.action === 'save' || fb.action === 'apply') {
                    feedbackMultiplier = 1.10; // +10% synergy
                } else if (fb.action === 'dismiss' || fb.action === 'skip') {
                    feedbackMultiplier = 0.85; // -15% penalty
                }
                break; // Use the most recent matching company logic
            }
        }
    }
