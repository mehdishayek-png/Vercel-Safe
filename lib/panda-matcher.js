...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    for (const skill of skills) {
        ...
        if (skill.length <= 4) {
            ...
            skillMatched = new RegExp(`\\b${escaped}\\b`).test(combined);
        } else {
            skillMatched = combined.includes(skill.normalized);
        }
        // Also check synonyms if direct match failed
        if (!skillMatched && SKILL_SYNONYMS[skill.normalized]) {
            skillMatched = SKILL_SYNONYMS[skill.normalized].some(syn => combined.includes(syn));
        }
        if (skillMatched) {
            matchedCount++;
            ...
            let baseValue = (Math.min(skill.length, SCORING_CONFIG.skill.lengthCap) * SCORING_CONFIG.skill.lengthMult) + (skill.caps * SCORING_CONFIG.skill.capsMult) + SCORING_CONFIG.skill.base;
    ...
    // TITLE AFFINITY BONUS — exact/near title match is the strongest relevance signal
    ...
    if (candidateTitleWords.length > 0 && titleOverlap.length > 0) {
        const titleRatio = titleOverlap.length / candidateTitleWords.length;
        const titleBonus = titleRatio * (SCORING_CONFIG.titleAffinity?.maxBonus || 30);
        keywordScore += titleBonus;
    }
    ...
    let finalScore = (keywordScore / SCORING_CONFIG.baseNormDivisor) * 100;
    ...
    if (!isExploreMode) {
        if (roleFamilyMultiplier <= SCORING_CONFIG.roleFamily.crossFamily) {
            finalScore = Math.min(finalScore, SCORING_CONFIG.caps.crossFamilyCap); // Cross-family mismatch hard cap
        } else if (roleFamilyMultiplier <= SCORING_CONFIG.roleFamily.unclassifiable) {
            finalScore = Math.min(finalScore, SCORING_CONFIG.caps.unclassifiedCap); // Unclassified or zero-overlap title cap
        }
    } else {
