...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    // 10. SEMANTIC EMBEDDING SIMILARITY (Marco Ferri inspired)
    // Computes cosine similarity between user role text and job text
    // Acts as a bonus signal — doesn't replace keyword scoring
    let semanticMultiplier = 1.0;
    let embeddingSimilarity = null;
    try {
        embeddingSimilarity = await computeSemanticMatch(profile, job);
    } catch {}

    if (embeddingSimilarity !== null) {
        // Blend: if embedding similarity is high, boost the score
        // If low, apply a specialty gate (like Marco Ferri's approach)
        if (embeddingSimilarity >= 0.65) {
            semanticMultiplier = 1.2; // Strong semantic match — boost
        } else if (embeddingSimilarity >= 0.50) {
            semanticMultiplier = 1.0; // Decent semantic match — neutral
        } else if (embeddingSimilarity >= 0.35) {
            semanticMultiplier = 0.90; // Weak match — slight penalty
        } else {
            semanticMultiplier = 0.75; // Poor semantic match — specialty gate
        }
    }
