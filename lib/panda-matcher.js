...
function computeDomainMultiplier(profileSkills, profileTitle, jobTitle, jobSummary) {
    ...
    // but we also can't confirm a match. Apply a mild uncertainty penalty (0.75)
    // rather than treating unknown as neutral (1.0). This prevents sparse job
    // descriptions with only cross-domain terms from escaping domain gating entirely.
    if (userClusters.size === 0 || jobClusters.size === 0) {
        return { multiplier: 0.75, reason: 'undetected', userClusters: [...userClusters], jobClusters: [...jobClusters] };
    }
