...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    if (userCity && cityMatchesInText(userCity, combined)) {
        ...
        // Remote/Anywhere but no country match — gentle debuff (many remote jobs don't list a country)
        locationMultiplier = 0.80;
    } else {
    ...
    return {
        score: Math.min(finalScore, 100), // Preserve float — round only once at final combined stage in matcher.js
