...
const ROLE_FAMILIES = {
    engineering: ['engineer', 'developer', 'programmer', 'architect', 'sde', 'swe', 'devops', 'sre', 'qa ', 'qe ', 'test engineer', 'software', 'fullstack', 'full stack', 'frontend', 'backend', 'iam engineer'],
    ...
    sales: ['sales', 'account executive', 'business development', 'bdr', 'sdr', 'sales engineer', 'revenue'],
...
export async function calculatePandaScore(job, profile, preferences = {}) {
    ...
    const isRemote = combined.includes('remote') || combined.includes('wfh') || combined.includes('work from home') || combined.includes('distributed');
    ...
    } else if (userCountryKey && jobCountryMatch === userCountryKey) {
        locationMultiplier = 1.0;  // Same country, different city — neutral
