...
// Google Jobs and job boards use both interchangeably
const CITY_ALIASES = {
...
/** Checks if any alias of a city name appears in the text */
...
const ROLE_FAMILIES = {
    ...
    cx_support: ['customer experience', 'customer success', 'customer support', 'cx ', 'csm', 'technical account manager', 'tam ', 'support specialist', 'customer care', 'client success', 'customer operations'],
    ...
    operations: ['operations', 'ops manager', 'supply chain', 'logistics', 'procurement'],
...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    // 2. GRANULAR SENIORITY HARD-COUNTER (5-level system)
    // Levels: exec(5) > senior(4) > manager(3) > mid(2) > entry(1) > open(0)
    const EXEC_WORDS = ['director', 'vp', 'vice president', 'principal', 'chief', 'cto', 'coo', 'ceo', 'cfo', 'head of', 'staff engineer', 'distinguished'];
    const SENIOR_WORDS = ['senior', 'sr ', 'sr.', 'lead', 'team lead'];
    const MANAGER_WORDS = ['manager', 'supervisor', 'program manager'];
    const ENTRY_WORDS = ['intern', 'internship', 'fresher', 'trainee', 'junior', 'entry'];

    function jobLevel(t) {
        if (EXEC_WORDS.some(w => t.includes(w))) return 5;
        if (SENIOR_WORDS.some(w => t.includes(w))) return 4;
        if (MANAGER_WORDS.some(w => t.includes(w))) return 3;
        return ENTRY_WORDS.some(w => t.includes(w)) ? 1 : 2; // default mid
    }
    ...
    const jLevel = jobLevel(title);
    const cLevel = candidateLevel(candidateYears);
    const gap = jLevel - cLevel; // positive = reaching up, negative = reaching down

    let seniorityMultiplier = 1.0;
    if (gap === 0) seniorityMultiplier = 1.25;       // Perfect level match
    else if (gap === 1) seniorityMultiplier = 0.85;   // Reaching up 1 — ambitious, fine
    else if (gap === 2) seniorityMultiplier = 0.35;   // Reaching up 2 — unlikely (e.g. 5yr → Senior Manager)
    else if (gap >= 3) seniorityMultiplier = 0.05;    // Reaching up 3+ — impossible
    else if (gap === -1) seniorityMultiplier = 0.80;  // Reaching down 1 — slight overqualified
    else if (gap === -2) seniorityMultiplier = 0.30;  // Reaching down 2 — too junior for them
    else if (gap <= -3) seniorityMultiplier = 0.05;   // Reaching down 3+ — insulting
    ...
    // ONLY check title + location for remote signals — NOT the full description.
    // Descriptions mention "remote" in passing ("remote teams", "remote collaboration")
    // which causes false positives that bypass all location penalties.
    ...
    if (userCity && !isRemote) {
        if (!cityMatchesInText(userCity, combined)) {
            // No alias of the user's city found in combined text
    ...
    // If either is unclassifiable, stay neutral (1.0x) — don't penalize unknown roles
    ...
    if (seniorityMultiplier <= 0.1) {
        finalScore = Math.min(finalScore, 15); // Impossible level gap — near-kill
    } else if (seniorityMultiplier <= 0.35) {
        finalScore = Math.min(finalScore, 40); // 2-level gap — hard cap
    }
