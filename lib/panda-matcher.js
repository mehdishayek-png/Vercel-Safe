...
const NICHE_TOOLS = new Set([
    ...
    // Customer Success
    'gainsight', 'totango', 'churnzero', 'planhat', 'vitally',
    // Design
    'figma', 'sketch', 'webflow', 'framer',
    // Observability
    ...
    // Payments
    'stripe', 'razorpay', 'adyen', 'braintree',
    // Comms
    'twilio', 'sendgrid', 'mailchimp',
    // Hosting
    'vercel', 'netlify', 'cloudflare', 'fastly',
    // AI/LLM
    'chatbot', 'claude', 'openai', 'langchain', 'llamaindex',
    // Game engines
    'unity', 'unreal', 'godot',
]);
...
const ROLE_FAMILIES = {
    ...
    operations: ['operations manager', 'ops manager', 'operations lead', 'operations specialist', 'operations analyst', 'product operations', 'automation lead', 'workflow automation', 'supply chain', 'logistics', 'procurement'],
    ...
    content: ['content strategist', 'content creator', 'copywriter', 'content manager', 'technical writer', 'documentation engineer', 'content lead', 'content specialist', 'content writer'],
    solutions_architecture: ['solutions architect', 'solution engineer', 'presales', 'pre-sales', 'sales engineer', 'implementation engineer', 'solutions engineer', 'professional services', 'implementation consultant'],
...
function computeDomainMultiplier(profileSkills, profileTitle, jobTitle, jobSummary) {
    ...
    // - If the job has no/short description: data gap, not a mismatch → neutral (1.0)
    // - If the job has content but no domain detected: sparse description → mild penalty (0.75)
    // This prevents punishing LinkedIn/ATS sources that return title-only results.
    if (userClusters.size === 0 || jobClusters.size === 0) {
        ...
        return { multiplier, reason: 'undetected', userClusters: [...userClusters], jobClusters: [...jobClusters] };
    }
...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    for (const skill of skills) {
        // Word-boundary matching for short skills to prevent false positives
        let skillMatched;
        let matchedViaSynonym = false;
        let synonymLength = 0;
        if (skill.length <= 4) {
            const escaped = skill.normalized.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            skillMatched = new RegExp(`\\b${escaped}\\b`).test(skillMatchText);
        } else {
    ...
    if (domainResult.reason === 'mismatch') {
        finalScore = Math.min(finalScore, isExploreMode ? SCORING_CONFIG.caps.domainMismatchCapExplore : SCORING_CONFIG.caps.domainMismatchCap);
    }
    if (!isExploreMode) {
