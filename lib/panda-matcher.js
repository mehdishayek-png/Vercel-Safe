...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    for (const skill of skills) {
        // Short skills (<=4 chars like "AI", "SQL", "AWS") only match in TITLE
        // to avoid false positives ("AI" inside "maintain", "email", etc.)
        const searchIn = skill.length <= 4 ? title : combined;
        if (searchIn.includes(skill.normalized)) {
            matchedCount++;
            ...
            // Tech acronym buff: Short, spaceless acronyms (sso, saml, okta, aws, sql)
            // Reduced from +15 to +8 since title-only matching already ensures relevance
