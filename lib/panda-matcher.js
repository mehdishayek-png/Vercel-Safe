...
export async function calculatePandaScore(job, profile, preferences = {}, apiKeys = {}) {
    ...
    if (candidateYears < 2) { // Junior/Entry candidate
        ...
        if (isManagerJob) seniorityMultiplier = 0.15; // Manager with <2y = unrealistic
        ...
        if (isManagerJob) seniorityMultiplier = 0.80; // Manager at 3-5y is a stretch in tech but normal in construction/ops
        if (isSeniorJob) seniorityMultiplier = 0.35; // Senior/Lead/Director with <6y = hard reach
        if (isInternJob) seniorityMultiplier = 0.05; // Hard counter: Insulting
    } else if (candidateYears >= 6 && candidateYears < 10) { // Manager-ready candidate
        if (isManagerJob) seniorityMultiplier = 1.2; // Sweet spot for manager roles
        if (isSeniorJob) seniorityMultiplier = 0.85; // Reaching up to senior
        if (isMidJob) seniorityMultiplier = 0.85; // Slightly overqualified
        if (isInternJob) seniorityMultiplier = 0.02; // Blocked
    } else { // Senior candidate (10+)
        if (isSeniorJob) seniorityMultiplier = 1.3;
        if (isManagerJob) seniorityMultiplier = 1.1; // Can manage easily
    ...
    if (userCity && cityMatchesInText(userCity, combined)) {
        ...
        locationMultiplier = 0.35;  // Same country, different city (fallback if explicitlyWrong didn't catch it)
