 ...
 * Production mode (Vercel runtime): writes directly to Redis ring buffer.
 *   Previously used an HTTP POST back to /api/debug/match-logs, which raced
 *   against a 2s timeout during active scans and silently dropped entries.
...
const isNode = typeof process !== 'undefined' && process.versions?.node && !isVercel;
...
// Lazy-load Redis only in Vercel runtime to keep this module safe for local Node
let redisClient;
...
async function writeRedis(entry) {
    try {
        ...
        if (process.env.DEBUG_MATCH_LOGGER) {
            console.warn('[match-logger] redis write failed:', err.message);
        }
        return false;
    }
}
