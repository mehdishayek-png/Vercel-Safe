    ...
    return (
        <button
            ...
            className={[
                ...
                disabled
                    ? 'border-ink-100 bg-surface-50 text-ink-300 cursor-not-allowed'
                    : active
                        ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-400'
                        : 'border-ink-200 dark:border-[#2E2B27] bg-white dark:bg-[#1C1B19] text-ink-600 dark:text-ink-300 hover:border-brand-300 hover:text-brand-600',
            ].join(' ')}
        >
            {label}
        </button>
    ...
    return (
        <div className={flagEnabled ? '' : 'opacity-40 pointer-events-none select-none'}>
            <div className="flex items-center gap-1.5 mb-2">
                <span className="text-[10px] tracking-widest text-ink-400 uppercase font-semibold">{title}</span>
                {!flagEnabled && <Lock className="w-3 h-3 text-ink-300" />}
            </div>
            {children}
        </div>
    );
...
export function FilterPanel({
    ...
    return (
        <div className="rounded-[10px] border border-ink-200 dark:border-[#2E2B27] bg-white dark:bg-[#1C1B19] overflow-hidden">
            <button
                type="button"
                onClick={() => setOpen(o => !o)}
                className="w-full flex items-center justify-between px-4 py-3 hover:bg-surface-50 dark:hover:bg-[#252420] transition-colors cursor-pointer"
            >
                <div className="flex items-center gap-2">
                    <SlidersHorizontal className="w-4 h-4 text-brand-500" />
                    <span className="text-sm font-medium text-ink-900 dark:text-ink-100">Filters</span>
                    {isActive && masterOn && (
                        <span className="text-[10px] font-bold bg-brand-500 text-white px-1.5 py-0.5 rounded-md">{activeCount}</span>
                    )}
                    {!masterOn && (
                ...
                <div className="flex items-center gap-2">
                    {isActive && masterOn && (
                        <button type="button" onClick={e => { e.stopPropagation(); reset(); }} className="text-[10px] text-ink-400 hover:text-red-500 flex items-center gap-1 transition-colors cursor-pointer">
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.15 }} className="overflow-hidden">
                        <div className="px-4 pb-4 space-y-4 border-t border-ink-100 pt-3">
                            ...
                            <FilterSection title="Region" flagEnabled={masterOn && flags?.MULTI_REGION}>
                                {masterOn && !flags?.MULTI_REGION && <p className="text-[10px] text-ink-400 mb-2">Multi-region coming soon.</p>}
                                <div className="flex flex-wrap gap-1.5">
                                    {REGIONS.map(val => (
                            ...
                            <FilterSection title="Salary Floor" flagEnabled={masterOn && flags?.SALARY_FILTER}>
                                {masterOn && !flags?.SALARY_FILTER && <p className="text-[10px] text-ink-400 mb-2">Salary filter coming soon.</p>}
                                <div className="flex gap-2">
                                    <input type="number" min={0} placeholder="Min salary..." value={filters.salaryMin ?? ''} onChange={e => setSalaryMin(e.target.value ? Number(e.target.value) : null)} disabled={!masterOn || !flags?.SALARY_FILTER}
                                        className="flex-1 text-xs px-3 py-2 border border-ink-200 dark:border-[#2E2B27] rounded-lg focus:outline-none focus:border-brand-400 disabled:bg-surface-50 disabled:text-ink-300 placeholder:text-ink-300 text-ink-900 dark:text-ink-100 bg-white dark:bg-[#1C1B19]" />
                                    <select value={filters.salaryCurrency || 'INR'} onChange={e => setSalaryCurrency(e.target.value)} disabled={!masterOn || !flags?.SALARY_FILTER}
                                        className="text-xs border border-ink-200 dark:border-[#2E2B27] rounded-lg px-2 py-2 focus:outline-none focus:border-brand-400 text-ink-700 dark:text-ink-200 bg-white dark:bg-[#1C1B19] disabled:text-ink-300 disabled:bg-surface-50">
                                        {SALARY_CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
                                    </select>
                                </div>
                                {masterOn && flags?.SALARY_FILTER && (
                                    <label className="flex items-center gap-2 mt-2 cursor-pointer">
                                        <input type="checkbox" checked={filters.includeMissingSalary} onChange={e => setIncludeMissingSalary(e.target.checked)} className="rounded border-ink-300 accent-brand-600" />
                                        <span className="text-[11px] text-ink-500">Include jobs without salary info</span>
                                    </label>
                                )}
                            </FilterSection>

                            <FilterSection title="Company Size" flagEnabled={masterOn && flags?.COMPANY_SIZE_FILTER}>
                                {masterOn && !flags?.COMPANY_SIZE_FILTER && <p className="text-[10px] text-ink-400 mb-2">Coming soon.</p>}
                                <div className="flex flex-wrap gap-1.5">
