...
function Tooltip({ text, children }) {
    ...
    return (
        ...
        >
            {children}
            {show && (
                <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 rounded-lg bg-ink-900 text-white text-[11px] leading-relaxed w-56 text-center shadow-lg z-50 pointer-events-none">
                    {text}
...
export function ScanControls({
    ...
    return (
        <div className="bg-white rounded-[10px] border border-ink-200 p-5 space-y-5">
            {/* Experience */}
            <div>
                <div className="flex justify-between items-baseline mb-2">
                    <span className="text-[10px] font-semibold tracking-widest text-ink-400 uppercase flex items-center gap-1">
                        Experience
            ...
            {/* Location */}
            <div>
                <div className="text-[10px] font-semibold tracking-widest text-ink-400 uppercase mb-2">Location</div>
                <Combobox
                    options={countries}
            ...
            <div className="flex flex-col gap-2">
                <label className={`flex items-center gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-colors text-xs font-medium ${
                    preferences.remoteOnly ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-surface-50 border-ink-200 text-ink-600 hover:bg-surface-100'
                }`}>
                    ...
                    <Tooltip text="Only show jobs that are listed as remote or work-from-home. Filters out office-based roles.">
                        <Info className="w-3.5 h-3.5 text-ink-400 ml-auto shrink-0" />
                    </Tooltip>
                </label>

                <label className={`flex items-center gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-colors text-xs ${
                    exploreAdjacent ? 'bg-violet-50 border-violet-200 text-violet-700' : 'bg-surface-50 border-ink-200 text-ink-600 hover:bg-surface-100'
                }`}>
                    ...
                    <Tooltip text="Loosens matching to include roles outside your exact career track. Great for career pivots or discovering unexpected fits.">
                        <Info className="w-3.5 h-3.5 text-ink-400 shrink-0" />
                    </Tooltip>
                </label>

                {tokensLoading ? (
                    <div className="flex items-center gap-2.5 p-2.5 rounded-lg border bg-surface-50 border-ink-200 animate-pulse">
                        <div className="w-4 h-4 rounded bg-surface-200" />
                        <div className="h-3 bg-surface-200 rounded w-24" />
                    </div>
                ) : (
                    <label className={`flex items-center gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-colors text-xs ${
                        midasSearch ? 'bg-accent-50 border-accent-200 text-accent-700' : 'bg-surface-50 border-ink-200 text-ink-600 hover:bg-surface-100'
            ...
            <div className="flex gap-2">
                <button onClick={onReset} className="flex-1 py-2.5 rounded-lg border border-ink-200 bg-white text-xs font-medium text-ink-500 hover:bg-surface-50 cursor-pointer transition-colors">
                    Reset
                </button>
                <button
                    onClick={findJobs}
                    disabled={isMatching}
                    className="flex-[2.5] py-2.5 rounded-lg border-none text-xs font-semibold text-white cursor-pointer bg-brand-500 hover:bg-brand-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {isMatching ? (
