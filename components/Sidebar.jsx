...
export function Sidebar({ isOpen, onClose }) {
    ...
    return (
        <>
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-[264px] bg-slate-50/70 dark:bg-slate-950/70 backdrop-blur-xl shadow-sidebar flex flex-col p-6 space-y-2 min-h-screen transform transition-transform duration-200 ease-in-out md:static md:translate-x-0 md:w-[264px] md:z-auto md:sticky md:top-0 md:shrink-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                {/* Logo */}
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800/50">
                        ...
                        <SignedIn>
                            <div className="flex items-center gap-2.5">
                                ...
                                <div className="min-w-0">
                                    <p className="text-[12px] font-medium text-gray-900 dark:text-gray-100 truncate">{user?.firstName || 'User'}</p>
                                    <p className="text-[10px] text-gray-400 dark:text-gray-500 truncate">{user?.primaryEmailAddress?.emailAddress || ''}</p>
                                </div>
                            </div>
                        </SignedIn>
                        <SignedOut>
                    ...
                    {/* Support banner */}
                    <div className="mt-4 mx-0 px-3 py-2.5 rounded-xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200/60 dark:border-amber-800/30">
                    ...
                    {/* Footer links */}
                    <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-gray-300 dark:text-gray-600 px-2">
