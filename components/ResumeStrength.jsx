    ...
    return (
        <div className="bg-white border border-surface-200 rounded-xl p-5 shadow-card">
            ...
            <div className="space-y-2">
                {checks.map((check, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                        className="flex items-center justify-between text-[13px] p-2 rounded-lg bg-surface-50 border border-surface-100"
                    >
                        <span className={`flex items-center gap-2 font-medium ${check.passed ? 'text-gray-700' : 'text-gray-400'}`}>
                        ...
                        {check.passed && <span className="text-[10px] font-semibold text-brand-500 bg-brand-50 px-1.5 py-0.5 rounded-md">+{check.points}</span>}
                    </motion.div>
                ))}
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-surface-100">
                ...
                <div onClick={initiatePayment} className="cursor-pointer bg-brand-50 border border-brand-100 hover:border-brand-200 rounded-lg p-3 transition-colors">
                    <div className="flex items-start gap-2.5">
                        <Sparkles className="w-4 h-4 text-brand-500 shrink-0 mt-0.5" />
                        <div>
                            <p className="text-xs font-semibold text-gray-900 mb-0.5">50x Deep Scan Pack</p>
